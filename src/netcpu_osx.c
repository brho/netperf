...
void get_host_ticks(host_cpu_load_info_t info)
{
	mach_msg_type_number_t count;

	count = HOST_CPU_LOAD_INFO_COUNT;
	host_statistics(lib_host_port, HOST_CPU_LOAD_INFO, (host_info_t) info,
					&count);
	return;
}
...
float calc_cpu_util_internal(float elapsed_time)
{
	...
	userticks =
		UNSIGNED_DIFFERENCE((lib_end_ticks.cpu_ticks[CPU_STATE_USER] +
							 lib_end_ticks.cpu_ticks[CPU_STATE_NICE]),
							(lib_start_ticks.cpu_ticks[CPU_STATE_USER] +
							 lib_start_ticks.cpu_ticks[CPU_STATE_NICE]));
	systicks =
		UNSIGNED_DIFFERENCE(lib_end_ticks.cpu_ticks[CPU_STATE_SYSTEM],
							lib_start_ticks.cpu_ticks[CPU_STATE_SYSTEM]);
	idleticks =
		UNSIGNED_DIFFERENCE(lib_end_ticks.cpu_ticks[CPU_STATE_IDLE],
							lib_start_ticks.cpu_ticks[CPU_STATE_IDLE]);
	totalticks = userticks + systicks + idleticks;

	lib_local_cpu_util =
		((float)userticks + (float)systicks) / (float)totalticks *100.0f;

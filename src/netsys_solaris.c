...
static void find_cpu_model_freq(char **cpu_model, int *frequency)
{
	...
	if ((NULL == ksp) || ((ksp) && (KSTAT_TYPE_NAMED != ksp->ks_type))) {
...
static void find_system_model_sysinfo(char **system_model)
{
...
int main(int argc, char *argv[])
{
	...
	printf("system_model %s, cpu_model %s, frequency %d\n",
		   system_model, cpu_model, frequency);
}
#endif

...
void get_cpu_idle(uint64_t * res)
{
	...
	if (perfstat_buffer == NULL) {
		fprintf(where, "cpu_start: malloc failed errno %d\n", errno);
		fflush(where);
		exit(-1);
	}
	...
	ret = perfstat_cpu(&name,
					   perfstat_buffer,
					   sizeof(perfstat_cpu_t), lib_num_loc_cpus);

	if ((ret == -1) || (ret != lib_num_loc_cpus)) {
		fprintf(where,
				"cpu_start: perfstat_cpu failed/count off; errno %d cpus %d count %d\n",
				errno, lib_num_loc_cpus, ret);
		fflush(where);
		exit(-1);
	}
...
{
	unsigned long long firstcnt[MAXCPUS], secondcnt[MAXCPUS];
	...
	perfstat_buffer = (perfstat_cpu_t *) malloc(lib_num_loc_cpus *
												sizeof(perfstat_cpu_t));
	if (perfstat_buffer == NULL) {
		fprintf(where, "calibrate_perfstat: malloc failed errno %d\n", errno);
		fflush(where);
		exit(-1);
	}
		...
		for (j = 0; j < lib_num_loc_cpus; j++) {
			...
			if (debug) {
				...
				fprintf(where,
						...
						j,
						firstcnt[j],
						firstcnt[j], j, secondcnt[j], secondcnt[j]);
			}
	...
	for (i = 0; i < lib_num_loc_cpus; i++) {
		...
		if (debug) {
			fprintf(where,
					...
					i,
					actual_rate, lib_local_maxrate, lib_local_per_cpu_util[i]);
		}
	}

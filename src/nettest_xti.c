...
void get_xti_info(socket, info_struct)
int socket;
...
SOCKET create_xti_endpoint(char *name)
{
	...
	if ((opt_req =
		 (struct t_optmgmt *)t_alloc(temp_socket, T_OPTMGMT, T_ALL)) == NULL) {
		fprintf(where,
	...
	if (debug) {
		fprintf(where,
				"netperf: create_xti_endpoint: socket sizes determined...\n");
...
void send_xti_tcp_stream(char remote_host[])
{
...
%6d\n";
	...
	xti_tcp_stream_request =
		(struct xti_tcp_stream_request_struct *)netperf_request.content.
		test_specific_data;
	xti_tcp_stream_response =
		(struct xti_tcp_stream_response_struct *)netperf_response.content.
		test_specific_data;
	xti_tcp_stream_result =
		(struct xti_tcp_stream_results_struct *)netperf_response.content.
		test_specific_data;
	...
	if (print_headers) {
		...
		if (iteration_max > 1) {
			fprintf(where,
					" : +/-%3.1f%% @ %2d%% conf.",
					interval / 0.02, confidence_level);
		}
	...
	if (local_cpu_usage || remote_cpu_usage) {
		...
		remote_cpu_method =
			format_cpu_method(xti_tcp_stream_result->cpu_method);
...
void recv_xti_tcp_stream()
{
	...
	if (debug) {
		fprintf(where,
				"recv_xti_tcp_stream: recv alignment and offset set...\n");
		fflush(where);
	}
	...
	while ((len = t_rcv(s_data,
						recv_ring->buffer_ptr, recv_size, &xti_flags)) != -1) {
		bytes_received += len;
		receive_calls++;
	...
	if (t_look(s_data) == T_ORDREL) {
		...
		fprintf(where,
				...
				errno, t_errno, len);
		fprintf(where, " t_look 0x%.4x", t_look(s_data));
		fflush(where);
		netperf_response.content.serv_errno = t_errno;
		send_response();
		exit(1);
...
void send_xti_tcp_rr(char remote_host[])
{
...
%5d  %5d   %5d  %5d\n";
	...
	xti_tcp_rr_request =
		(struct xti_tcp_rr_request_struct *)netperf_request.content.
		test_specific_data;
	xti_tcp_rr_response =
		(struct xti_tcp_rr_response_struct *)netperf_response.content.
		test_specific_data;
	xti_tcp_rr_result =
		(struct xti_tcp_rr_results_struct *)netperf_response.content.
		test_specific_data;
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		if (debug > 1) {
			fprintf(where,
					"netperf: send_xti_tcp_rr: requesting TCP rr test\n");
		}
...

void send_xti_udp_stream(char remote_host[])
{
  /**********************************************************************/
	/*                                    */
...
%6d           %-7.2f   %7d           %7.1f     %-6.2f   %-6.3f\n\n";
	...
	float elapsed_time,
		recv_elapsed, local_cpu_utilization, remote_cpu_utilization;
	...
	xti_udp_stream_request =
		(struct xti_udp_stream_request_struct *)netperf_request.content.
		test_specific_data;
	xti_udp_stream_response =
		(struct xti_udp_stream_response_struct *)netperf_response.content.
		test_specific_data;
	xti_udp_stream_results =
		(struct xti_udp_stream_results_struct *)netperf_response.content.
		test_specific_data;
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		if (!netperf_response.content.serv_errno) {
			if (debug)
				fprintf(where,
						"send_xti_udp_stream: remote results obtained\n");
		} else {
			Set_errno(netperf_response.content.serv_errno);
			perror("send_xti_udp_stream: error on remote");
			exit(1);
	...
	if (local_cpu_usage || remote_cpu_usage) {
		local_cpu_method = format_cpu_method(cpu_method);
		remote_cpu_method =
			format_cpu_method(xti_udp_stream_results->cpu_method);
...
void recv_xti_udp_stream()
{
	...
	if (debug > 1) {
		fprintf(where,
				"recv_xti_udp_stream: receive alignment and offset set...\n");
		fflush(where);
	}
	...
	while (!times_up) {
		...
		if (debug) {
			...
			fprintf(where, "udatamax %d udatalen %d udatabuf %x\n",
					...
					unitdata.udata.len, unitdata.udata.buf);
			fflush(where);
		}
#endif /* RAJ_DEBUG */
...
%5d  %5d   %5d  %5d\n";
	...
	xti_udp_rr_request =
		(struct xti_udp_rr_request_struct *)netperf_request.content.
		test_specific_data;
	xti_udp_rr_response =
		(struct xti_udp_rr_response_struct *)netperf_response.content.
		test_specific_data;
	xti_udp_rr_result =
		(struct xti_udp_rr_results_struct *)netperf_response.content.
		test_specific_data;
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		if (debug > 1) {
			fprintf(where,
					"netperf: send_xti_udp_rr: requesting UDP r/r test\n");
		}
...
void recv_xti_udp_rr()
{
	...
	if (debug) {
		fprintf(where,
				"recv_xti_udp_rr: requested recv alignment of %d offset %d\n",
				xti_udp_rr_request->recv_alignment,
				xti_udp_rr_request->recv_offset);
		fprintf(where,
				"recv_xti_udp_rr: requested send alignment of %d offset %d\n",
	...
	if (debug) {
		fprintf(where,
				"recv_xti_udp_rr: receive alignment and offset set...\n");
		fflush(where);
	}
...
void recv_xti_tcp_rr()
{
	...
	if (debug) {
		fprintf(where,
				"recv_xti_tcp_rr: requested recv alignment of %d offset %d\n",
				xti_tcp_rr_request->recv_alignment,
				xti_tcp_rr_request->recv_offset);
		fprintf(where,
				"recv_xti_tcp_rr: requested send alignment of %d offset %d\n",
	...
	if (xti_tcp_rr_request->measure_cpu) {
		...
		xti_tcp_rr_response->cpu_rate =
			calibrate_local_cpu(xti_tcp_rr_request->cpu_rate);
	}
...
void send_xti_tcp_conn_rr(char remote_host[])
{
...
%5d  %5d   %5d  %5d\n";
	...
	xti_tcp_conn_rr_request =
		(struct xti_tcp_conn_rr_request_struct *)netperf_request.content.
		test_specific_data;
	xti_tcp_conn_rr_response =
		(struct xti_tcp_conn_rr_response_struct *)netperf_response.content.
		test_specific_data;
	xti_tcp_conn_rr_result =
		(struct xti_tcp_conn_rr_results_struct *)netperf_response.content.
		test_specific_data;
	...
	if (debug > 1) {
		fprintf(where,
				"netperf: send_xti_tcp_conn_rr: requesting TCP crr test\n");
	}
...
void recv_xti_tcp_conn_rr()
{
	...
	if (debug) {
		fprintf(where,
				"recv_xti_tcp_conn_rr: receive alignment and offset set...\n");
		fflush(where);
	}
...
void scan_xti_args(int argc, char *argv[])
{

...
SOCKET create_unix_socket(int family, int type)
{
	...
	if (temp_socket == INVALID_SOCKET) {
		fprintf(where, "netperf: create_unix_socket: socket: %d\n", errno);
		fflush(where);
		exit(1);
	}
...
void send_stream_stream(char remote_host[])
{
...
#include <sys/stat.h>
	...
	stream_stream_request =
		(struct stream_stream_request_struct *)netperf_request.content.
		test_specific_data;
	stream_stream_response =
		(struct stream_stream_response_struct *)netperf_response.content.
		test_specific_data;
	stream_stream_result =
		(struct stream_stream_results_struct *)netperf_response.content.
		test_specific_data;
	...
	/*set up the data socket                        */
	send_socket = create_unix_socket(AF_UNIX, SOCK_STREAM);
...
void recv_stream_stream()
{
	...
	bzero((char *)&myaddr_un, sizeof(myaddr_un));
	...
	s_listen = create_unix_socket(AF_UNIX, SOCK_STREAM);
	...
	if (debug) {
		fprintf(where,
				"recv_stream_stream: receive alignment and offset set...\n");
		fflush(where);
	}
	...
	if (getsockname(s_listen,
					(struct sockaddr *)&myaddr_un, &addrlen) == SOCKET_ERROR) {
		netperf_response.content.serv_errno = errno;
		close(s_listen);
		send_response();
	...
	if (debug) {
		fprintf(where, "recv_stream_stream: got %d bytes\n", bytes_received);
		fprintf(where, "recv_stream_stream: got %d recvs\n", receive_calls);
		fflush(where);
	}
...
void send_stream_rr(char remote_host[])
{
...
%5d  %5d   %5d  %5d\n";
	...
	stream_rr_request =
		(struct stream_rr_request_struct *)netperf_request.content.
		test_specific_data;
	stream_rr_response =
		(struct stream_rr_response_struct *)netperf_response.content.
		test_specific_data;
	stream_rr_result =
		(struct stream_rr_results_struct *)netperf_response.content.
		test_specific_data;
...

void send_dg_stream(char remote_host[])
{
  /*********************************************************************/
	/*                                       */
...
%5d            %-7.2f   %7d           %7.1f      %-6.2f  %-6.3f\n\n";
	...
	dg_stream_request =
		(struct dg_stream_request_struct *)netperf_request.content.
		test_specific_data;
	dg_stream_response =
		(struct dg_stream_response_struct *)netperf_response.content.
		test_specific_data;
	dg_stream_results =
		(struct dg_stream_results_struct *)netperf_response.content.
		test_specific_data;
	...
	/*set up the data socket          */
	data_socket = create_unix_socket(AF_UNIX, SOCK_DGRAM);
	...
	if (send_size == 0) {
		if (lss_size > 0) {
			send_size =
				(lss_size < UNIX_LENGTH_MAX ? lss_size : UNIX_LENGTH_MAX);
		} else {
			send_size = 4096;
		}
	}
...
void recv_dg_stream()
{
	...
	s_data = create_unix_socket(AF_UNIX, SOCK_DGRAM);
	...
	if (debug) {
		fprintf(where, "recv_dg_stream: got %d bytes\n", bytes_received);
		fflush(where);
	}
...

void send_dg_rr(char remote_host[])
{
...
#define	MAX_KEPT_TIMES	1024
	...
	dg_rr_request =
		(struct dg_rr_request_struct *)netperf_request.content.
		test_specific_data;
	dg_rr_response =
		(struct dg_rr_response_struct *)netperf_response.content.
		test_specific_data;
	dg_rr_result =
		(struct dg_rr_results_struct *)netperf_response.content.
		test_specific_data;
	...
	/*set up the data socket                        */
	send_socket = create_unix_socket(AF_UNIX, SOCK_DGRAM);
	...
	if (debug > 1) {
		fprintf(where,
				"netperf: send_dg_rr: requesting DG request/response test\n");
	}
...
#endif
		if ((len = send(send_socket,
						send_message_ptr, req_size, 0)) != req_size) {
			if (errno == EINTR) {
				/* We likely hit */
				/* test-end time. */
				break;
...
void recv_dg_rr()
{
	...
	if (debug) {
		fprintf(where, "recv_dg_rr: requested recv alignment of %d offset %d\n",
				dg_rr_request->recv_alignment, dg_rr_request->recv_offset);
	...
	while ((!times_up) || (trans_remaining > 0)) {
		...
		fprintf(where, "socket %d ptr %p size %d\n",
				s_data, recv_ring->buffer_ptr, dg_rr_request->request_size);
		fflush(where);
		if (recvfrom(s_data,
					 recv_ring->buffer_ptr,
					 dg_rr_request->request_size,
	...
	if (debug) {
		fprintf(where, "recv_dg_rr: got %d transactions\n", trans_received);
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_dg_rr: test complete, sending results.\n");
		fflush(where);
	}
...
void recv_stream_rr()
{
	...
	if (debug) {
		fprintf(where,
				"recv_stream_rr: requested recv alignment of %d offset %d\n",
				stream_rr_request->recv_alignment,
				stream_rr_request->recv_offset);
		fprintf(where,
				"recv_stream_rr: requested send alignment of %d offset %d\n",
	...
	if (stream_rr_request->measure_cpu) {
		...
		stream_rr_response->cpu_rate =
			calibrate_local_cpu(stream_rr_request->cpu_rate);
	}
	...
	if (debug) {
		fprintf(where,
				"recv_stream_rr: accept completes on the data connection.\n");
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_stream_rr: got %d transactions\n", trans_received);
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_stream_rr: test complete, sending results.\n");
		fflush(where);
	}
...
void scan_unix_args(int argc, char *argv[])
{

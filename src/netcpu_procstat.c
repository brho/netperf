	...
	if (!proc_stat_buf) {
		...
		if (debug) {
			fprintf(where,
					...
					CPU_LINE_LENGTH, proc_stat_buflen);
			fflush(where);
		}
...
float calc_cpu_util_internal(float elapsed_time)
{
	...
	if (debug) {
		fprintf(where, "lib_local_maxrate = %f\n", lib_local_maxrate);
	}
	for (i = 0; i < lib_num_loc_cpus; i++) {
		...
		diff.hard_irq =
			tick_subtract(lib_start_count[i].hard_irq,
						  lib_end_count[i].hard_irq);
		diff.soft_irq =
			tick_subtract(lib_start_count[i].soft_irq,
						  lib_end_count[i].soft_irq);
		diff.steal =

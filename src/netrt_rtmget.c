...
char *find_egress_interface_by_addr(struct sockaddr *addr, int local_ip_check)
{
...
#if defined(AF_LINK)
char *find_egress_interface_by_link(struct sockaddr_dl *socklink)
{
	...
	printf("socklink asdf index %d nlen %d alen %d slen %d\n",
		   socklink->sdl_index,
		   socklink->sdl_nlen, socklink->sdl_alen, socklink->sdl_slen);
#endif
	...
	do {
		...
		if (ret < sizeof(struct rt_msghdr)) {
			...
			return strdup("read");
			 ...
			 rtm->rtm_seq != 12865 || rtm->rtm_pid != getpid());
	if ((rtm->rtm_flags & RTF_GATEWAY) && (rtm->rtm_addrs & RTA_GATEWAY)) {

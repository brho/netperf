...
int debug = 0,					/* debugging level */
	...
	keep_histogram = 0, keep_statistics = 0;
...
/* cpu variables */
int local_cpu_usage = 0,		/* you guessed it */
	remote_cpu_usage = 0;	/* still right ! */

float local_cpu_rate = 0.0F, remote_cpu_rate = 0.0F;
...
int test_time = 10,				/* test ends by time */
...
int local_recv_align = 8,		/* alignment for local receives */
	local_send_align = 8,	/* alignment for local sends */
	local_send_offset = 0, local_recv_offset = 0, remote_recv_align = 8,	/* alignment for remote receives */
	remote_send_align = 8,	/* alignment for remote sends */
	remote_send_offset = 0,
	remote_recv_offset = 0, remote_send_width = 0, remote_recv_width = 0;

/* hoist above the if for omni */
int interval_usecs = 0,
	interval_wate = 0,
	interval_burst = 0, remote_interval_usecs = 0, remote_interval_burst = 0;
...
int loc_dirty_count = 0,
	loc_clean_count = 0, rem_dirty_count = 0, rem_clean_count = 0;
#else
int loc_dirty_count = -1,
	loc_clean_count = -1, rem_dirty_count = -1, rem_clean_count = -1;
#endif /* DIRTY */
...
#endif
	if (strstr(temp, "inet") || strstr(temp, "4")) {
		return (AF_INET);
	}
#if defined(AF_RDS)
	if (strstr(temp, "af_rds") || strstr(temp, "32")) {
		return (AF_RDS);
	}
#endif
	if (strstr(temp, "unspec") || strstr(temp, "0")) {
		return (AF_UNSPEC);
	}
	fprintf(where,
...

void print_netserver_usage()
{
	fprintf(stderr, "%s", netserver_usage);
}

void print_netperf_usage()
{
		...
		switch (c) {
			...
			case 'I':
				...
				if ((confidence_level != 95) && (confidence_level != 99)) {
					printf
						("Only 95%% and 99%% confidence level is supported\n");
					exit(-1);
				}
				if (arg2[0]) {
...
#else
					fprintf(where, "Packet rate control is not compiled in.\n");
#endif
				}
				if (arg2[0]) {
...
#else
					fprintf(where, "Packet burst size is not compiled in. \n");
			...
			case 'B':
				...
				if (NULL != result_brand) {
					...
					fprintf(where, "Unable to malloc space for result brand\n");
				}
				break;
			case '4':
...
#else
				printf
					("This netperf was not compiled on an IPv6 capable system!\n");
				exit(-1);
#endif
				break;
			case 'V':
	...
	if (strcasecmp(test_name, "omni")) {
		if ('?' == libfmt) {
			...
			if ((strstr(test_name, "RR")) ||
				(strstr(test_name, "rr")) ||
				(strstr(test_name, "Rr")) || (strstr(test_name, "rR"))) {
				libfmt = 'x';
			} else {
				libfmt = 'm';
			}

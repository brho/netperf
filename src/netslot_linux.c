				   ...
				   int *sub_vend, int *sub_dev)
{
	...
	/* first the vendor id */
	ret =
		snprintf(sysfile, 128, "/sys/class/net/%s/device/vendor",
				 interface_name);
	*vendor = get_val_from_file(sysfile);

	/* next the device */
	ret =
		snprintf(sysfile, 128, "/sys/class/net/%s/device/device",
				 interface_name);
	*device = get_val_from_file(sysfile);

	/* next the subsystem vendor */
	ret =
		snprintf(sysfile, 128, "/sys/class/net/%s/device/subsystem_vendor",
				 interface_name);
	*sub_vend = get_val_from_file(sysfile);

	/* next the subsystem device */
	ret =
		snprintf(sysfile, 128, "/sys/class/net/%s/device/subsystem_device",
				 interface_name);

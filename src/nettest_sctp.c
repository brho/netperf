...
static
void get_sctp_info(int socket, int *mss)
{

	socklen_t sock_opt_len;

	if (sctp_opt_info(socket, 0, SCTP_MAXSEG, mss, &sock_opt_len) < 0) {
		lss_size = -1;
	}
}

static
void sctp_enable_events(int socket, int ev_mask)
{
	...
	if (setsockopt(socket, IPPROTO_SCTP,
#ifdef SCTP_EVENTS
				   SCTP_EVENTS,
#else
				   SCTP_SET_EVENTS,
#endif
				   (const char *)&ev, sizeof(ev)) != 0) {
		fprintf(where,
...

static sctp_disposition_t sctp_process_event(int socket, void *buf)
{
...
void send_sctp_stream(char remote_host[])
{
...
%6d\n";
	...
	sctp_stream_request =
		(struct sctp_stream_request_struct *)netperf_request.content.
		test_specific_data;
	sctp_stream_response =
		(struct sctp_stream_response_struct *)netperf_response.content.
		test_specific_data;
	sctp_stream_result =
		(struct sctp_stream_results_struct *)netperf_response.content.
		test_specific_data;
	...
	complete_addrinfos(&remote_res,
					   &local_res, remote_host, SOCK_STREAM, IPPROTO_SCTP, 0);
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		if (!netperf_response.content.serv_errno) {
			...
			set_port_number(remote_res,
							(short)sctp_stream_response->data_port_number);
...
void recv_sctp_stream(void)
{
	...
	if (debug) {
		fprintf(where,
				"recv_sctp_stream: set recv_size = %d, align = %d, offset = %d.\n",
	...
	if (getsockname(s_listen, (struct sockaddr *)&myaddr_in, &addrlen) == -1) {
		netperf_response.content.serv_errno = errno;
		close(s_listen);
		send_response();
...
void send_sctp_stream_1toMany(char remote_host[])
{
...
%6d\n";
	...
	complete_addrinfos(&remote_res,
					   &local_res,
					   remote_host, SOCK_SEQPACKET, IPPROTO_SCTP, 0);

	if (print_headers) {
		print_top_test_header("SCTP 1-TO-MANY STREAM TEST", local_res,
							  remote_res);
	}
	...
	if (send_socket == NULL) {
		fprintf(where,
				"send_sctp_stream_1toMany: failed to allocation sockets!\n");
		exit(1);
	}
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		if (!netperf_response.content.serv_errno) {
			...
			/* network order */
			set_port_number(remote_res,
							(unsigned short)sctp_stream_response->
							data_port_number);
			rem_rcvavoid = sctp_stream_response->so_rcvavoid;
			rem_sndavoid = sctp_stream_response->so_sndavoid;
		} else {
		...
		for (j = 0; j < num_associations; j++) {
			...
			if (send_socket[j] < 0) {
				perror
					("netperf: send_sctp_stream_1toMany: sctp stream data socket");
				exit(1);
			}

			if (debug) {
				fprintf(where,
						"send_sctp_stream_1toMany: send_socket obtained...\n");
			}
			...
			if (connect(send_socket[j],
						...
						remote_res->ai_addrlen) == INVALID_SOCKET) {
				perror
					("netperf: send_sctp_stream_1toMany: data socket connect failed");
				exit(1);
			}
		...
		if (non_block) {
			...
			select(send_socket[num_associations - 1] + 1, &readfds, NULL, NULL,
				   NULL);
		} else {
...
void recv_sctp_stream_1toMany(void)
{
	...
	if (debug) {
		fprintf(where,
				"recv_sctp_stream_1toMany: setting the response type...\n");
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where,
				"recv_sctp_stream_1toMany: the response type is set...\n");
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where,
				"recv_sctp_stream: receive alignment and offset set...\n");
		fflush(where);
	}
	...
	/* now get the port number assigned by the system  */
	addrlen = sizeof(myaddr_in);
	if (getsockname(s_recv, (struct sockaddr *)&myaddr_in, &addrlen) == -1) {
		netperf_response.content.serv_errno = errno;
		close(s_recv);
		send_response();
	...
	while ((len = sctp_recvmsg(s_recv, recv_ring->buffer_ptr, recv_size, NULL, 0,	/* we don't care who it's from */
							   NULL, &msg_flags)) != 0) {
		if (len < 0) {
			if (non_block && errno == EAGAIN) {
				Set_errno(0);
...
void send_sctp_rr(char remote_host[])
{
...
%5d  %5d   %5d  %5d\n";
	...
	sctp_rr_request =
		(struct sctp_rr_request_struct *)netperf_request.content.
		test_specific_data;
	sctp_rr_response =
		(struct sctp_rr_response_struct *)netperf_response.content.
		test_specific_data;
	sctp_rr_result =
		(struct sctp_rr_results_struct *)netperf_response.content.
		test_specific_data;
	...
	if (print_headers) {
		print_top_test_header("SCTP REQUEST/RESPONSE TEST", local_res,
							  remote_res);
	}
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		/*Connect up to the remote port on the data socket  */
		if (connect(send_socket,
					remote_res->ai_addr, remote_res->ai_addrlen) < 0) {
...
#ifdef WANT_FIRST_BURST
		{
			...
			for (i = 0; i < first_burst_size; i++) {
				if ((len = sctp_sendmsg(send_socket, send_ring->buffer_ptr, req_size, NULL, 0,	/* don't need addrs with 1-to-1 */
										0, 0, 0, 0, 0)) != req_size) {
		...
		while ((!times_up) || (trans_remaining > 0)) {
			...
			while ((len = sctp_sendmsg(send_socket, send_ring->buffer_ptr, req_size, NULL, 0,	/* don't need addrs with 1-to-1 */
									   0, 0, 0, 0, 0)) != req_size) {
				if (non_block && errno == EAGAIN) {
					/* try sending again */
					continue;
...
void recv_sctp_rr(void)
{
	...
	if (debug) {
		fprintf(where,
				"recv_sctp_rr: requested recv alignment of %d offset %d\n",
				sctp_rr_request->recv_alignment, sctp_rr_request->recv_offset);
		fprintf(where,
				"recv_sctp_rr: requested send alignment of %d offset %d\n",
	...
	if (sctp_rr_request->measure_cpu) {
		...
		sctp_rr_response->cpu_rate =
			calibrate_local_cpu(sctp_rr_request->cpu_rate);
	}
	...
	if ((s_data = accept(s_listen,
						 (struct sockaddr *)&peeraddr_in, &addrlen)) == -1) {
		/* Let's just punt. The remote will be given some information */
		close(s_listen);
	...
	if (debug) {
		fprintf(where,
				"recv_sctp_rr: accept completes on the data connection.\n");
		fflush(where);
	}
...
void send_sctp_rr_1toMany(char remote_host[])
{
...
%5d  %5d   %5d  %5d\n";
	...
	if (print_headers) {
		print_top_test_header("SCTP 1-TO-MANY REQUEST/RESPONSE TEST", local_res,
							  remote_res);
	}
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		if (test_time) {
			...
			sctp_rr_request->test_length = test_trans * num_associations * -1;
		}
		sctp_rr_request->non_blocking = non_block;
		...
		if (debug > 1) {
			fprintf(where,
					"netperf: send_sctp_rr_1toMany: requesting SCTP rr test\n");
		}
		...
		for (j = 0; j < num_associations; j++) {
			...
			if (send_socket < 0) {
				perror
					("netperf: send_sctp_rr_1toMany: sctp stream data socket");
				exit(1);
			}
			...
			for (j = 0; j < num_associations; j++) {
				for (i = 0; i < first_burst_size; i++) {
					if ((len = sctp_sendmsg(send_socket[j],
						...
						/* we should never hit the end of the test in the first burst */
						perror
							("send_sctp_rr_1toMany: initial burst data send error");
						exit(1);
					}
				}
			}
...

 /* this routine implements the receive (netserver) side of a TCP_RR */
 /* test */
void recv_sctp_rr_1toMany(void)
{
	...
	if (debug) {
		fprintf(where,
				"recv_sctp_rr_1toMany: requested recv alignment of %d offset %d\n",
				sctp_rr_request->recv_alignment, sctp_rr_request->recv_offset);
		fprintf(where,
				"recv_sctp_rr_1toMany: requested send alignment of %d offset %d\n",
				sctp_rr_request->send_alignment, sctp_rr_request->send_offset);
		fflush(where);
	}
	...
	/* now get the port number assigned by the system  */
	addrlen = sizeof(myaddr_in);
	if (getsockname(s_rcv, (struct sockaddr *)&myaddr_in, &addrlen) == -1) {
		netperf_response.content.serv_errno = errno;
		close(s_rcv);
		send_response();
...
void scan_sctp_args(int argc, char *argv[])
{

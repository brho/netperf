...
void cpu_util_init(void)
{
	...
	for (ksp = kc->kc_chain, i = 0;
		 (ksp != NULL) && (i < MAXCPUS); ksp = ksp->ks_next) {
...
static void print_unexpected_statistic_warning(char *who, char *what, char *why)
{
	if (why) {
		fprintf(where,
		...
		fprintf(where, "%s is ignoring statistic %.16s\n", who, what);
	}
}
...
static void get_cpu_time_counters(cpu_time_counters_t * counters)
{
...
#define CALC_ACCURACY CALC_THOUSANDTH_PERCENT
	...
	for (i = 0; i < lib_num_loc_cpus; i++) {
		...
		total_cpu_nsec =
			...
			delta_cpu_counters[i].user + delta_cpu_counters[i].kernel;
		...
		interrupt_kernel =
			((delta_cpu_counters[i].interrupt * fraction_kernel) /
			 CALC_ACCURACY);
		...
		if (debug) {
			fprintf(where,
					...
					interrupt_idle,
					fraction_user,
					interrupt_user, fraction_kernel, interrupt_kernel);
		}
		...
		fraction_interrupt =
			(corrected_cpu_counters[i].interrupt * CALC_ACCURACY) /
			total_cpu_nsec;

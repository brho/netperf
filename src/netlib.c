...
void PrintWin32Error(FILE * stream, LPSTR text)
{
	...
	dwResult =
		FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER |
					  ...
					  FORMAT_MESSAGE_ARGUMENT_ARRAY, NULL, dwError,
					  LANG_NEUTRAL, (LPTSTR) & szTemp, 0, NULL);
...
int inet_nton(int af, const void *src, char *dst, int cnt)
{
...
#endif
		default:
			fprintf(where, "Unexpected Address Family of %u\n", res->ai_family);
			fflush(where);
			exit(-1);
	}
}
...
int netperf_sendfile(SOCKET send_socket, struct ring_elt *send_ring)
{
	...
	len = sendfile(send_socket, send_ring->fildes, &scratch_offset,	/* modified after the call! */
				   send_ring->length);
#elif defined (__sun)
	...
	ret = sendfile(send_ring->fildes,
				   send_socket,
				   send_ring->offset,
				   send_ring->length, NULL, (off_t *) & len, send_ring->flags);
#elif defined(USE_OSX)
	len = send_ring->length;
	ret = sendfile(send_ring->fildes,
				   send_socket,
				   send_ring->offset, (off_t *) & len, NULL, send_ring->flags);
#else /* original sendile HP-UX */
	len = sendfile(send_socket,
				   send_ring->fildes,
				   send_ring->offset,
				   send_ring->length, send_ring->hdtrl, send_ring->flags);
#endif
...
void gettimeofday(struct timeval *tv, struct timezone *not_used)
{
	FT nt_time;
...
#ifdef WIN32
#define SIGALRM (14)
void emulate_alarm(int seconds)
{
	DWORD ErrorCode;
	...
	if (ErrorCode == WAIT_TIMEOUT) {
		...
		/* Give the other threads time to notice that times_up has
		   ...
		   sockets. */
		timed_out = 0;
		if (WaitForSingleObject(hAlarm, PAD_TIME / 2 * 1000) == WAIT_TIMEOUT) {
			timed_out = 1;
...
void start_timer(int time)
{
	...
	if (hAlarm == (HANDLE) INVALID_HANDLE_VALUE) {
		...
		hAlarm = CreateEvent((LPSECURITY_ATTRIBUTES) NULL,	/* no security */
	...
	tHandle = CreateThread(0,
						   ...
						   (LPVOID) (ULONG_PTR) time, 0, &thread_id);
	CloseHandle(tHandle);
...
/* this routine will enable the interval timer and set things up so
   ...
   days */
void start_itimer(unsigned int interval_len_msec)
{
#ifdef WIN32
	...
	if (mmr == TIMERR_NOERROR) {
		if (interval_len_msec < ptc.wPeriodMin) {
			...
			fprintf(where,
					"Timer cannot be set to %dmsec.  Minimum timer resolution: %d\n",
	...
	if (!SetWaitableTimer
		(WinTimer, &liDueTime, interval_len_msec, NULL, NULL, TRUE)) {
	...
	ticks_per_itvl = ((interval_wate * sysconf(_SC_CLK_TCK) * 1000) / 1000000);
	...
	if (debug) {
		fprintf(where,
				...
				usec_per_itvl % 1000000, test_len_ticks);
		fflush(where);
	}
...
access_buffer(char *buffer_ptr, int length, int dirty_count, int clean_count)
{
	...
	for (i = 0; ((i < dirty_count) && (temp_buffer < limit)); i++) {
		*temp_buffer += (char)i;
		dirty_totals += *temp_buffer;
		temp_buffer++;
	}

	for (i = 0; ((i < clean_count) && (temp_buffer < limit)); i++) {
		dirty_totals += *temp_buffer;
		temp_buffer++;
	}
...
struct ring_elt *allocate_exs_buffer_ring(int width, int buffer_size,
										  int alignment, int offset,
										  exs_mhandle_t * mhandlep)
{
	...
	*mhandlep =
		exs_mregister((void *)mmap_buffer_aligned, (size_t) mmap_size, 0);
	...
	for (i = 0, temp_link = first_link; i < width; i++, temp_link++) {
		...
		temp_link->buffer_base =
			(char *)mmap_buffer_aligned + (i * malloc_size);
#ifndef WIN32
...
struct ring_elt *alloc_sendfile_buf_ring(int width,
										 int buffer_size,
										 int alignment, int offset)
{
	...
	if (strcmp(local_fill_file, "") == 0) {
		...
		if (temp_buffer) {
			...
			perror
				("alloc_sendfile_buf_ring: could not allocate buffer for file");
			exit(-1);
		}
	} else {
...
void dump_response()
{
	...
	for (counter = 0; counter < ((sizeof(netperf_response) / 4) - 3);
		 counter += 4) {
		fprintf(where,
				"%d:\t%8x %8x %8x %8x \t>%4.4s< >%4.4s< >%4.4s< >%4.4s<\n",
...
char *format_number(double number)
{
	...
	switch (libfmt) {
		...
		case 'K':
			...
			break;
		case 'M':
			snprintf(fmtbuf, sizeof(fmtbuf), "%-7.2f",
		...
		case 'm':
			snprintf(fmtbuf, sizeof(fmtbuf), "%-7.2f",
					 number * 8 / 1000.0 / 1000.0);
			break;
		case 'g':
			snprintf(fmtbuf, sizeof(fmtbuf), "%-7.2f",
					 number * 8 / 1000.0 / 1000.0 / 1000.0);
			break;
		case 'x':
			snprintf(fmtbuf, sizeof(fmtbuf), "%-7.2f", number);
			break;
...
void shutdown_control()
{
	...
	if (shutdown(netlib_control, 1) == SOCKET_ERROR) {
		Print_errno(where, "shutdown_control: error in shutdown");
		fflush(where);
		exit(1);
	}
...
// No select
	...
	if (select(FD_SETSIZE, &readfds, 0, 0, &timeout) != 1) {
		Print_errno(where, "shutdown_control: no response received");
		fflush(where);
		exit(1);
	}
#endif
...
#define NETPERF_CPU_ZERO(cpusetp) *cpusetp = (unsigned long)0
	...
	if (mapped_affinity < 8 * sizeof(netperf_cpu_set)) {
		...
		if (sched_setaffinity(getpid(), len, &netperf_cpu_set)) {
			if (debug) {
				fprintf(stderr,
						"failed to set PID %d's CPU affinity errno %d\n",
						getpid(), errno);
				fflush(stderr);
			}
		}
...
#include <sys/processor.h>
...

	{
		...
		if ((mapped_affinity < 0) || (mapped_affinity > MAXIMUM_PROCESSORS)) {
			fprintf(where,
...
#else
	if (debug) {
		fprintf(where, "Processor affinity not available for this platform!\n");
		fflush(where);
	}
#endif
}
...
void send_request_n(int n)
{
	...
	if (send(netlib_control,
			 ...
			 sizeof(netperf_request), 0) != sizeof(netperf_request)) {
...
void send_response_n(int n)
{
	...
	if (debug > 1) {
		fprintf(where,
				...
				(unsigned int)sizeof(netperf_response) / 4, count);
		dump_response();
	}
...
{
	int tot_bytes_recvd, bytes_recvd, bytes_left;
	...
	do {
		...
		if (select(FD_SETSIZE,
				   &readfds, 0, 0, (seconds > 0) ? &timeout : NULL) != 1) {
			fprintf(where,
	...
	} while ((tot_bytes_recvd != buflen) && (bytes_recvd > 0));
	...
	if (bytes_recvd == SOCKET_ERROR) {
		Print_errno(where, "recv_request: error on recv");
		fflush(where);
		close(server_sock);
		return -1;
	}
...
{
	int tot_bytes_recvd, bytes_recvd = 0, bytes_left;
	char *buf = (char *)&netperf_response;
	int buflen = sizeof(netperf_response);
	int counter, count;
	...
	for (counter = 0;
		 counter < sizeof(netperf_response) / sizeof(int); counter++) {
	...
	if ((counter = select(FD_SETSIZE, &readfds, 0, 0, &timeout)) != 1) {
		fprintf(where,
				"%s: no response received. errno %d counter %d\n",
				__FUNCTION__, errno, counter);
		exit(1);
	}
#endif
...
#if 0
		token = (char *)netperf_response.content.test_specific_data + (sizeof(netperf_response) - 7);	/* OBOB? */
		*token = 0;
#endif

		token =
			strtok((char *)netperf_response.content.test_specific_data, delim);
		if (token)
			remote_sysname = strdup(token);
		else
			remote_sysname = strdup("UnknownRemoteSysname");
		token = strtok(NULL, delim);
		if (token)
			remote_release = strdup(token);
		else
			remote_release = strdup("UnknownRemoteRelease");
		token = strtok(NULL, delim);
		if (token)
			remote_machine = strdup(token);
		else
			remote_machine = strdup("UnknownRemoteMachine");
		token = strtok(NULL, delim);
		if (token)
			remote_version = strdup(token);
		else
...
void
set_sock_buffer(SOCKET sd, enum sock_buffer which, int requested_size,
				int *effective_sizep)
{
#ifdef SO_SNDBUF
	...
	if (requested_size >= 0) {
		if (setsockopt(sd, SOL_SOCKET, optname,
			...
			fprintf(where,
					"netperf: set_sock_buffer: %s option: errno %d (%s)\n",
					(which == SEND_BUFFER) ? "SO_SNDBUF" : "SO_RCVBUF", errno,
					strerror(errno));
			fflush(where);
			exit(1);
			  ...
			  const char *host, char *port, int family)
{
	...
	while (temp) {
		...
		fprintf(dumploc,
				...
				inet_ptos(temp->ai_protocol), temp->ai_addrlen);
		ai_addr = temp->ai_addr;
		if (ai_addr != NULL) {
			int i;
			fprintf(dumploc,
					"\tsa_family: %s sadata:", inet_ftos(ai_addr->sa_family));
...
struct addrinfo *resolve_host(char *hostname, char *port, int family)
{
	...
	count = 0;
	do {
		error = getaddrinfo((char *)hostname, (char *)port, &hints, &ai);
		count += 1;
		if (error == EAI_AGAIN) {
			if (debug) {
	...
	if (error) {
		printf("%s: could not resolve host '%s' port '%s' af %s"
			   ...
			   __FUNCTION__,
			   hostname, port, inet_ftos(family), error, gai_strerror(error));
		return (NULL);
	}
...
establish_control_internal(char *hostname,
						   char *port,
						   int remfam,
						   char *localhost, char *localport, int locfam)
{
	int not_connected;
	SOCKET control_sock;
	...
	if (debug) {
		fprintf(where,
				...
				hostname,
				port,
				inet_ftos(remfam), localhost, localport, inet_ftos(locfam));
		fflush(where);
	}
	...
	while (remote_res_temp != NULL) {
		...
		/* I am guessing that we should use the address family of the
		   ...
		   2003-02-26 */
		control_sock = socket(local_res_temp->ai_family, SOCK_STREAM, 0);
		...
		if (bind(control_sock,
				 local_res_temp->ai_addr, local_res_temp->ai_addrlen) == 0) {
			if (debug) {
				fprintf(where,
...
establish_control(char *hostname,
				  char *port,
				  int remfam, char *localhost, char *localport, int locfam)
{

	netlib_control = establish_control_internal(hostname,
												port,
												remfam,
												localhost, localport, locfam);
...
char *get_id()
{
	...
	snprintf(id_string, sizeof(id_string),
			 ...
			 system_name,
			 GetVersion() & 0xFF,
			 GetVersion() & 0xFF00, SystemInfo.dwProcessorType
#else
			 "%-15s%-15s%-15s%-15s%-15s",
			 system_name.sysname,
			 system_name.nodename,
			 system_name.release, system_name.version, system_name.machine
#endif /* WIN32 */
		);
	return (id_string);
}
...

float calc_cpu_util(float elapsed_time)
{
	float temp_util;
	int i;
							 ...
							 float cpu_utilization, int num_cpus)
{
	...
	if (debug) {
		fprintf(where,
				...
				"                             cpu_util = %f\n"
				"                             num cpu = %d\n",
				units_sent, elapsed_time, cpu_utilization, num_cpus);
		fflush(where);
	}
	...
	thruput = (units_sent / (double)unit_divisor / (double)elapsed_time);
	...
	/* thruput has a "per second" component. if we were using 100% (
	   ...
	   raj 12/95 */
	service_demand = (cpu_utilization * 10000.0 / thruput) * (float)num_cpus;
	...
	if (debug) {
		fprintf(where,
				...
				units_sent,
				elapsed_time,
				cpu_utilization, num_cpus, thruput, service_demand);
		fflush(where);
	}
...

float calibrate_local_cpu(float local_cpu_rate)
{
...
float calibrate_remote_cpu()
{
	...
	if (netperf_response.content.serv_errno) {
		...
		bcopy((char *)netperf_response.content.test_specific_data,
			  (char *)&remrate, sizeof(remrate));
		bcopy((char *)netperf_response.content.test_specific_data +
			  sizeof(remrate), (char *)&lib_num_rem_cpus,
...
{
	if (debug) {
		fprintf(where, "Resetting interim results\n");
		fflush(where);
	}
	units_this_tick = 0;
	...
	switch (netperf_output_mode) {
		...
		case KEYVAL:
			fprintf(where,
					...
					count,
					actual_interval / 1000000.0,
					count, now.tv_sec, (long)now.tv_usec / 1000);
			count += 1;
			break;
		default:
			fprintf(where,
...
#elif defined(WIN32)
	h->time_ones = (LARGE_INTEGER *) malloc(h->limit * sizeof(LARGE_INTEGER));
#else
	h->time_ones = (struct timeval *)malloc(h->limit * sizeof(struct timeval));
...
void HIST_add(register HIST h, int time_delta)
{
	...
	if (val < 10)
		h->unit_usec[(int)(val * base)]++;
	else {
		val /= 10;
		if (val < 10)
			h->ten_usec[(int)(val * base)]++;
		else {
			val /= 10;
			if (val < 10)
				h->hundred_usec[(int)(val * base)]++;
			else {
				val /= 10;
				if (val < 10)
					h->unit_msec[(int)(val * base)]++;
				else {
					val /= 10;
					if (val < 10)
						h->ten_msec[(int)(val * base)]++;
					else {
						val /= 10;
						if (val < 10)
							h->hundred_msec[(int)(val * base)]++;
						else {
							val /= 10;
							if (val < 10)
								h->unit_sec[(int)(val * base)]++;
							else {
								val /= 10;
								if (val < 10)
									h->ten_sec[(int)(val * base)]++;
								else
									h->ridiculous++;
							}
						}
...
double
	...
	measured_fails, measured_local_results, confidence = -10.0;
/*  interval=0.1; */
...
void init_stat()
{
	...
	measured_fails = 0.0;
	measured_local_results = 0.0, confidence = -10.0;
}
					 ...
					 float loc_cpu, float rem_cpu, float loc_sd, float rem_sd)
{
	...
	if (debug) {
		fprintf(where,
				...
				time, result, loc_cpu, rem_cpu, loc_sd, rem_sd);
		fflush(where);
	}

	/* the test time */
	measured_sum_local_time += (double)time;
	measured_square_sum_local_time += (double)time *time;
	...
	/* the test result */
	measured_sum_result += (double)result;
	measured_square_sum_result += (double)result *result;
	measured_mean_result = (double)measured_sum_result / confidence_iterations;
	measured_var_result =
	...
	measured_square_sum_local_cpu += (double)loc_cpu *loc_cpu;
	...
	measured_square_sum_remote_cpu += (double)rem_cpu *rem_cpu;
	...
	measured_square_sum_local_service_demand += (double)loc_sd *loc_sd;
	...
	measured_square_sum_remote_service_demand += (double)rem_sd *rem_sd;
	...
	if (confidence_iterations > 1) {
		...
		if (debug) {
			printf
				("Conf_itvl %2d: results:%4.1f%% loc_cpu:%4.1f%% rem_cpu:%4.1f%%\n",
				 confidence_iterations, (interval - result_confid) * 100.0,
		...
		if (!result_confidence_only) {
			confidence =
				min(min(result_confid, loc_cpu_confid), rem_cpu_confid);
		} else {

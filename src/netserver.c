...
static void init_netserver_globals()
{
...
#define NETPERF_NULL "nul"
	...
	if (NULL == rd_null_fp) {
		fprintf(where,
				...
				__FUNCTION__, NETPERF_NULL, strerror(errno), errno);
		fflush(where);
		exit(1);
	}
...
void create_listens(char hostname[], char port[], int af)
{
	...
	if (debug) {
		fprintf(stderr,
				...
				__FUNCTION__, hostname, port, inet_ftos(af), af);
		fflush(stderr);
	}
	...
	count = 0;
	do {
		error = getaddrinfo((char *)hostname, (char *)port, &hints, &local_res);
		count += 1;
		if (error == EAI_AGAIN) {
			if (debug) {
				fprintf(stderr,
	...
	if (error) {
		if (debug) {
			...
			fprintf(stderr,
					...
					__FUNCTION__,
					hostname, port, af, gai_strerror(error), error);
...
void setup_listens(char name[], char port[], int af)
{
	...
	if (listen_list) {
		fprintf(stdout,
				...
				(no_name) ? "IN(6)ADDR_ANY" : name, port, inet_ftos(af));
		fflush(stdout);
	} else {
		fprintf(stderr,
...
SOCKET set_fdset(struct listen_elt *list, fd_set * fdset)
{
	...
	while (temp) {
		...
		if (debug) {
			fprintf(where, "setting %d in fdset\n", temp->fd);
			fflush(where);
		}
		...
		switch (netperf_request.content.request_type) {
			...
			case DO_SYSINFO:
				{
					...
					for (i = 0; i < 4; i++) {
						if ((!strstr(local_sysname, delims[i])) &&
							...
							snprintf((char *)netperf_response.content.
									 test_specific_data,
									 sizeof(netperf_response.content.
											test_specific_data),
									 "%c%s%c%s%c%s%c%s", delims[i][0],
									 local_sysname, delims[i][0], local_release,
									 delims[i][0], local_machine, delims[i][0],
									 local_version);
							break;
						}
					}
					if (i == 4) {
						...
						snprintf((char *)netperf_response.content.
								 test_specific_data,
								 sizeof(netperf_response.content.
										test_specific_data), "%c%s%c%s%c%s%c%s",
								 delims[i][0], "NoDelimUnique", delims[i][0],
								 "NoDelimUnique", delims[i][0], "NoDelimUnique",
								 delims[i][0], "NoDelimUnique");
					}
					send_response_n(0);
					break;
				}
			case CPU_CALIBRATE:
				...
				bcopy((char *)&lib_num_loc_cpus,
					  (char *)netperf_response.content.test_specific_data +
					  sizeof(temp_rate), sizeof(lib_num_loc_cpus));
				if (debug) {
					fprintf(where,
			...
			case PASSPHRASE:
				if (debug) {
					fprintf(where,
							"Ignoring an unexpected passphrase control message\n");
					fflush(where);
				}
				break;
...
void spawn_child()
{
	...
	if (verbosity > 1) {
		cmd_index += snprintf(&cmdline[cmd_index],
							  cmdline_length - cmd_index, " -v %d", verbosity);
	}
	for (i = 0; i < debug; i++) {
		cmd_index += snprintf(&cmdline[cmd_index],
							  cmdline_length - cmd_index, " -d");
	}
	cmd_index += snprintf(&cmdline[cmd_index],
						  cmdline_length - cmd_index,
						  " -I %x", (int)(UINT_PTR) server_sock);

	/* are these -i settings even necessary? the command line scanning
	   does not seem to do anything with them */
	cmd_index += snprintf(&cmdline[cmd_index],
						  cmdline_length - cmd_index,
						  " -i %x", (int)(UINT_PTR) server_control);
	cmd_index += snprintf(&cmdline[cmd_index],
						  cmdline_length - cmd_index,
						  " -i %x", (int)(UINT_PTR) where);

	b = CreateProcess(NULL,	/* Application Name */
					  cmdline, NULL,	/* Process security attributes */
...
void accept_connection(SOCKET listen_fd)
{
	...
	setsockopt(server_sock,
			   SOL_SOCKET, SO_KEEPALIVE, (const char *)&on, sizeof(on));
#endif
	...
	while ((c = getopt(argc, argv, SERVER_ARGS)) != EOF) {
		switch (c) {
			...
			case 'Z':
				...
				strncpy(passphrase, optarg,
						sizeof(netperf_request.content.test_specific_data));
				passphrase[sizeof(netperf_request.content.test_specific_data) -
						   1] = '\0';
				break;
			case '4':

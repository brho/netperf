...
int loc_nodelay,				/* don't/do use NODELAY locally     */
...
extern int loc_tcpcork, rem_tcpcork, local_connected, remote_connected;
...
#endif
		default:
			fprintf(where, "Unexpected Address Family %u\n", res->ai_family);
			fflush(where);
			exit(-1);
	}
}
...
static void set_multicast_ttl(SOCKET sock)
{
	...
	if (multicast_ttl >= 0) {
		if (setsockopt(sock,
			...
			fprintf(where, "setsockopt(IP_TTL) failed errno %d\n", errno);
		}
	}
	if (getsockopt(sock,
				   IPPROTO_IP,
		...
		fprintf(where, "getsockopt(IP_TTL) failed errno %d\n", errno);
		multicast_ttl = -2;
	}
}
...
static void join_multicast_addr(SOCKET sock, struct addrinfo *res)
{
	switch (res->ai_family) {
		case AF_INET:{
				struct ip_mreq mreq;
		...
		case AF_INET6:{
				fprintf(where,
						"I do not know how to join an IPv6 multicast group\n");
				break;
			}
...
static void
extract_inet_address_and_port(struct addrinfo *res, void *addr, int len,
							  int *port)
{
	switch (res->ai_family) {
		case AF_INET:{
		...
		case AF_INET6:{
				struct sockaddr_in6 *foo = (struct sockaddr_in6 *)res->ai_addr;
				*port = foo->sin6_port;
				memcpy(addr, &(foo->sin6_addr),
					   min(len, sizeof(foo->sin6_addr)));
				break;
			}
#endif
		default:
...
pick_next_port_number(struct addrinfo *local_res, struct addrinfo *remote_res)
{
	...
	if (myport == get_port_number(remote_res))
		myport++;
...
void print_netperf_output_entry(FILE * where, enum netperf_output_name what)
{
}

void print_omni_init_list();

void dump_netperf_output_list(FILE * where)
{
...
void dump_netperf_output_choices(FILE * where, int csv)
{
	int i;
	...
	for (i = OUTPUT_NONE; i < NETPERF_OUTPUT_MAX; i++) {
		...
		fprintf(where,
				"%s",
				netperf_output_enum_to_str(netperf_output_source[i].
										   output_name));
	}
	fprintf(where, "\n");
	fflush(where);
...
void dump_netperf_output_source(FILE * where)
{
	int i;
	...
	for (i = OUTPUT_NONE; i < NETPERF_OUTPUT_MAX; i++) {
		fprintf(where,
				...
				(netperf_output_source[i].line[0] ==
				 NULL) ? "" : netperf_output_source[i].line[0],
				(netperf_output_source[i].line[1] ==
				 NULL) ? "" : netperf_output_source[i].line[1],
				(netperf_output_source[i].line[2] ==
				 NULL) ? "" : netperf_output_source[i].line[2],
				(netperf_output_source[i].line[3] ==
				 NULL) ? "" : netperf_output_source[i].line[3],
				(netperf_output_source[i].format ==
				 NULL) ? "" : netperf_output_source[i].format);
	}
	fflush(where);
}
	...
	for (name = OUTPUT_NONE; name < NETPERF_OUTPUT_MAX; name++) {
		...
		if (h1 != NULL) {
			for (k = 0; ((k < 4) &&
						 ...
						 (strcmp("", netperf_output_source[name].line[k])));
				 k++) {
				len = sprintf(h1, "%s", netperf_output_source[name].line[k]);
				*(h1 + len) = ' ';
...
void parse_output_selection_line(int line, char *list)
{
	...
	if (line >= NETPERF_MAX_BLOCKS) {
		if (debug) {
			fprintf(where,
					...
					NETPERF_MAX_BLOCKS, line + 1, list);
			fflush(where);
		}
		return;
	}
	...
	while ((token) && (j < NETPERF_OUTPUT_MAX)) {
		...
		if ((name == NETPERF_OUTPUT_UNKNOWN) && (debug)) {
			fprintf(where,
					"Ignoring unknown output selector %d |%s| on line %d\n",
					j + 1, token, line + 1);
			fflush(where);
		} else {
...
void parse_output_selection_direct(char *output_selection)
{
	...
	if ((NULL == source) || (NULL == line) || (NULL == remainder)) {
		fprintf(where,
				"Unable to malloc memory for output selection parsing\n");
		fflush(where);
		exit(-1);
	}
...
void set_output_list_by_test()
{
	...
	if (NETPERF_XMIT_ONLY(direction)) {
		...
		if (debug) {
			fprintf(where,
					"Cannot determine default test output, using mins\n");
			fflush(where);
		}
...
set_output_elt(enum netperf_output_name name,
			   ...
			   unsigned int group, enum netperf_output_type type)
{
...
void print_omni_init_list()
{
	...
	set_output_elt(REMOTE_CPU_CONFID, "Remote", "CPU", "Confidence",
				   "Width (%)", "%.3f", &rem_cpu_confid_pct, 1, 0,
				   NETPERF_TYPE_DOUBLE);

	set_output_elt(RT_LATENCY, "Round", "Trip", "Latency", "usec/tran", "%.3f",
				   &rtt_latency, 1, 0, NETPERF_TYPE_DOUBLE);

	set_output_elt(TRANSACTION_RATE, "Transaction", "Rate", "Tran/s", "",
				   "%.3f", &transaction_rate, 1, 0, NETPERF_TYPE_DOUBLE);
	...
	set_output_elt(LOCAL_BYTES_PER_RECV, "Local", "Bytes", "Per", "Recv",
				   "%.2f", &bytes_per_recv, 1, 0, NETPERF_TYPE_DOUBLE);

	set_output_elt(LOCAL_BYTES_PER_SEND, "Local", "Bytes", "Per", "Send",
				   "%.2f", &bytes_per_send, 1, 0, NETPERF_TYPE_DOUBLE);

	set_output_elt(LOCAL_BYTES_RECVD, "Local", "Bytes", "Received", "",
				   "%" PRIu64, &bytes_received, 1, 0, NETPERF_TYPE_UINT64);
	...
	set_output_elt(REMOTE_BYTES_RECVD, "Remote", "Bytes", "Received", "",
				   "%" PRIu64, &remote_bytes_received, 1, 0,
				   NETPERF_TYPE_UINT64);
	...
	set_output_elt(LOCAL_CPU_FREQUENCY, "Local", "CPU", "Frequency", "MHz",
				   "%d", &local_cpu_frequency, 1, 0, NETPERF_TYPE_INT32);
...
int
my_long_long_snprintf(char *buffer, size_t size, const char *format,
					  void *value)
{
...
int my_long_snprintf(char *buffer, size_t size, const char *format, void *value)
{
	const char *fmt = format;
	while (*fmt)
		switch (*fmt++) {
...
int old_my_snprintf(char *buffer, size_t size, const char *format, void *value)
{
	const char *fmt = format;
...
void print_omni_csv()
{
	...
	if (((hdr1 == NULL) && (print_headers)) || (val1 == NULL)) {
	...
	for (i = 0; i < NETPERF_MAX_BLOCKS; i++) {
		for (j = 0;
			...
			if ((netperf_output_source[output_list[i][j]].format != NULL) &&
				...
				fprintf(where,
						"%s=%s\n",
						netperf_output_enum_to_str(output_list[i][j]), tmpval);
			}
		}
	}
	fflush(where);
		...
		for (j = 0;
			...
			if (print_headers) {
				for (k = 0; k < 4; k++) {
					memcpy(h[k],
						   netperf_output_source[output_list[i][j]].line[k],
						   strlen(netperf_output_source[output_list[i][j]].
								  line[k]));
				}
			}
			if ((netperf_output_source[output_list[i][j]].format != NULL) &&
...
int connect_data_socket(SOCKET send_socket, struct addrinfo *remote_res)
{
	int ret;
...
static
int send_pktinfo(SOCKET data_socket, char *buffer, int len,
...
send_data(SOCKET data_socket, struct ring_elt *send_ring,
		  uint32_t bytes_to_send, struct sockaddr *destination, int destlen,
		  int protocol)
{
	...
	if (debug > 2) {
		fprintf(where,
				...
				__FUNCTION__,
				data_socket, send_ring, bytes_to_send, destination, destlen);
		fflush(where);
	}
	...
	if (destination) {
		if (have_pktinfo) {
			len = send_pktinfo(data_socket,
							   ...
							   bytes_to_send, destination, destlen);
		} else {
			len = sendto(data_socket, send_ring->buffer_ptr, bytes_to_send,
#if defined(MSG_FASTOPEN)
						 (use_fastopen
						  && protocol == IPPROTO_TCP) ? MSG_FASTOPEN : 0,
#else
						 0,
#endif
						 destination, destlen);
		...
		if (!use_write) {
			len = send(data_socket, send_ring->buffer_ptr, bytes_to_send, 0);
		} else {
#ifndef WIN32
			len = write(data_socket, send_ring->buffer_ptr, bytes_to_send);
#else
...
recv_data_no_copy(SOCKET data_socket, struct ring_elt *recv_ring,
				  uint32_t bytes_to_recv, struct sockaddr *source,
				  netperf_socklen_t * sourcelen, uint32_t flags,
...
#define SPLICE_F_NONBLOCK 0x02
	...
	if (debug > 1) {
		fprintf(where,
				...
				bytes_to_recv,
				source,
				(source != NULL) ? *sourcelen : -1, flags, num_receives);
		fflush(where);
	}
	do {

		bytes_recvd = splice(data_socket,
							 NULL, pfd[1], NULL, bytes_left, my_flags);
...
static
int recv_pktinfo(SOCKET data_socket, char *message_ptr, int bytes_to_recv,
				 int my_flags, struct sockaddr *source,
				 netperf_socklen_t * sourcelen)
{
	...
	if (ret >= 0) {
		...
		for (cmsg = CMSG_FIRSTHDR(&my_header);
			 cmsg != NULL; cmsg = CMSG_NXTHDR(&my_header, cmsg)) {
...
int
recv_data(SOCKET data_socket, struct ring_elt *recv_ring,
		  uint32_t bytes_to_recv, struct sockaddr *source,
		  netperf_socklen_t * sourcelen, uint32_t flags,
		  uint32_t * num_receives)
{
	...
	if (loc_rcvavoid == 1) {
		ret =
			recv_data_no_copy(data_socket, recv_ring, bytes_to_recv, source,
							  sourcelen, flags, num_receives);
		if (ret != -4)
			return ret;
		else
			loc_rcvavoid = 0;
		...
		if (source) {
			...
			if (use_pktinfo) {
				bytes_recvd = recv_pktinfo(data_socket,
										   temp_message_ptr,
										   bytes_left,
										   my_flags, source, sourcelen);
				use_pktinfo = 0;
			} else {
				bytes_recvd = recvfrom(data_socket,
									   temp_message_ptr,
									   bytes_left, my_flags, source, sourcelen);
			}
		} else {
			/* just call recv */
			bytes_recvd = recv(data_socket,
							   temp_message_ptr, bytes_left, my_flags);
		}
		if (bytes_recvd > 0) {
			bytes_left -= bytes_recvd;
	...
	*num_receives = my_recvs;
	...
	if (bytes_recvd < 0) {
		...
		if (SOCKET_EAGAIN(bytes_recvd) || SOCKET_EWOULDBLOCK(bytes_recvd)) {
			return -2;
		}
		/* it was a hard error */
		return -3;
				  ...
				  int protocol)
{
	...
	if (debug) {
		fprintf(where,
				...
				__FUNCTION__, data_socket, peer, peerlen, protocol);
		fflush(where);
	}
	...
	if (protocol == IPPROTO_UDP) {
		...
		for (i = 0; i < 3; i++) {
			if (peer)
				ret = sendto(data_socket, buffer, 0, 0, peer, peerlen);
			else
				ret = send(data_socket, buffer, 0, 0);
			if (SOCKET_EINTR(ret)) {
				close(data_socket);
				return -1;
			}
					   ...
					   struct sockaddr *peer, int peerlen)
{
	...
	if (debug) {
		fprintf(where,
				...
				__FUNCTION__, data_socket, initiate, do_close, protocol);
		fflush(where);
	}
	...
	if (protocol != IPPROTO_UDP) {
		...
		bytes_recvd = recv(data_socket, buffer, 1, 0);
		...
		for (i = 0; i < 3; i++) {
			if (peer)
				bytes_recvd = sendto(data_socket, buffer, 0, 0, peer, peerlen);
			else
				bytes_recvd = send(data_socket, buffer, 0, 0);
...
static int get_unsent_data(SOCKET socket, int protocol)
{
...
static int get_transport_retrans(SOCKET socket, int protocol)
{
	...
	if ((ret =
		 getsockopt(socket, protocol, TCP_INFO, &tcp_info, &infosize)) < 0) {
		if (debug) {
			fprintf(where,
...
static void get_transport_info(SOCKET socket, int *mss, int protocol)
{
	...
	if (getsockopt(socket,
				   protocol,
				   option, (char *)mss, &sock_opt_len) == SOCKET_ERROR) {
		fprintf(where, "%s: getsockopt: errno %d\n", __FUNCTION__, errno);
		fflush(where);
		*mss = -1;
	}
...
static void
get_transport_cong_control(SOCKET socket, int protocol, char cong_control[],
						   int len)
{
...
static void
set_transport_cong_control(SOCKET socket, int protocol, char cong_control[],
						   int len)
{
#ifdef TCP_CONGESTION
	if (protocol == IPPROTO_TCP) {
...
static void set_receive_timeout(SOCKET sock, int timeout)
{
#ifdef SO_RCVTIMEO
#ifndef WIN32
	struct timeval foo;
	...
	if (setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, &foo, sizeof(foo)) < 0) {
		if (debug) {
			fprintf(where,
					"Note - attempt to set a receive timeout on the data socket failed with errno %d (%s)\n",
					errno, strerror(errno));
			fflush(where);
		}
	}
...
static SOCKET omni_create_data_socket(struct addrinfo *res)
{
	SOCKET temp_socket;
	...
	if (temp_socket != SOCKET_ERROR) {
		...
		if ((res->ai_protocol == IPPROTO_UDP) && (receive_timeout != -1)) {
			set_receive_timeout(temp_socket, receive_timeout);
		}

		if (socket_debug) {
			int one = 1;
			setsockopt(temp_socket, SOL_SOCKET, SO_DEBUG, &one, sizeof(one));
		}
	}
	return temp_socket;
}
...
static int choose_send_size(int lss, int protocol)
{
...
void
send_omni_inner(char remote_host[], unsigned int legacy_caller,
				char header_str[])
{
...
#define REQUEST_CWND_INITIAL 2
	...
	omni_request =
		(struct omni_request_struct *)netperf_request.content.
		test_specific_data;
	omni_response =
		(struct omni_response_struct *)netperf_response.content.
		test_specific_data;
	omni_result =
		(struct omni_results_struct *)netperf_response.content.
		test_specific_data;
	...
	complete_addrinfos(&remote_res,
					   &local_res, remote_host, socket_type, protocol, 0);
	...
	while (((confidence < 0) && (confidence_iteration <= iteration_max)) ||
		...
		if ((protocol == IPPROTO_UDP) && (direction & NETPERF_XMIT)) {
		...
		if (!no_control) {
			...
			ret = get_sockaddr_family_addr_port(&my_addr,
												nf_to_af(omni_request->
														 ipfamily),
												omni_request->netperf_ip,
												&(omni_request->netperf_port));
			ret =
				get_sockaddr_family_addr_port((struct sockaddr_storage *)
											  remote_res->ai_addr,
			...
			strncpy(omni_request->fill_file,
					remote_fill_file, sizeof(omni_request->fill_file));
			...
			if (!netperf_response.content.serv_errno) {
				...
				if (NULL == remote_system_model) {
					omni_response->
						system_model[sizeof(omni_response->system_model) - 1] =
						0;
					remote_system_model = strdup(omni_response->system_model);
				}
				if (NULL == remote_cpu_model) {
					omni_response->cpu_model[sizeof(omni_response->cpu_model) -
											 1] = 0;
				...
				if (NULL == remote_security_specific) {
					omni_response->
						security_string[sizeof(omni_response->security_string) -
										1] = 0;
					remote_security_specific =
						strdup(omni_response->security_string);
				}
				/* top bits type, bottom bits enabled */
				remote_security_type_id =
					(int)omni_response->security_info >> 16;
				remote_security_enabled_num =
					(short)omni_response->security_info;
				remote_security_type =
					nsec_type_to_str(remote_security_type_id);
				remote_security_enabled =
...
again:
			...
			if (need_to_connect && !use_fastopen) {
				...
				if (ret == 0) {
					...
					if (debug) {
						fprintf(where,
								"transient! transient! torpedo in the water!\n");
						fflush(where);
					}
					close(data_socket);
			...
			while ((first_burst_size > 0) &&
				   ...
				   (NETPERF_IS_RR(direction)) && (!connection_test)) {
				if (debug > 1) {
					fprintf(where,
							"injecting, req_outstanding %d req_cwnd %d burst %d\n",
							requests_outstanding,
							request_cwnd, first_burst_size);
				}
			...
			if (direction & NETPERF_XMIT) {
				...
				ret = send_data(data_socket,
								...
								remote_res->ai_addrlen, protocol);
			...
			if (direction & NETPERF_RECV) {
				...
				if (rret > 0) {
					...
					if (debug) {
						fprintf(where,
								"Timeout receiving resonse from remote\n");
						fflush(where);
					}
#ifdef WANT_FIRST_BURST
					if (first_burst_size) {
				...
				if ((request_cwnd < first_burst_size) &&
					...
					if (debug > 1) {
						fprintf(where,
								"incr req_cwnd to %d first_burst %d reqs_outstanding %d trans %"
								PRIu64 "\n", request_cwnd, first_burst_size,
								requests_outstanding, trans_completed + 1);
					}
				}
			...
			if (connection_test) {
				...
				if (transport_mss == -2)
					get_transport_info(data_socket,
									   &transport_mss, local_res->ai_protocol);

				ret = disconnect_data_socket(data_socket,
											 (no_control) ? 1 : 0, 1, NULL, 0);
				if (ret == 0) {
		...
		local_security_enabled =
			nsec_enabled_to_str(local_security_enabled_num);
		...
		if (connected) {
			...
			/* CHECK PARMS HERE; */
			ret = disconnect_data_socket(data_socket, 1, 1, NULL, 0);
			connected = 0;
			need_socket = 1;
		...
		if (!legacy) {
			...
			find_system_info(&local_system_model,
							 &local_cpu_model, &local_cpu_frequency);
			...
			if ((desired_output_groups & OMNI_WANT_LOC_IFNAME) ||
				...
				local_interface_name =
					find_egress_interface(local_res->ai_addr,
										  remote_res->ai_addr);
			} else {
			...
			if (desired_output_groups & OMNI_WANT_LOC_IFSLOT) {
				local_interface_slot =
					find_interface_slot(local_interface_name);
			} else {
		...
		if (local_receive_calls > 0)
			bytes_per_recv =
				(double)bytes_received / (double)local_receive_calls;
		else
			bytes_per_recv = 0.0;
		...
		if (NETPERF_IS_RR(direction)) {
			...
			if (interval_burst) {
				rtt_elapsed_time -=
					(float)(interval_wait_microseconds / 1000000.0);
			}
#endif /* WANT_INTERVALS */
			...
			if (!connection_test) {
				...
				rtt_latency =
					(((double)1.0 / (trans_completed / rtt_elapsed_time)) *
					 (double)1000000.0) *
					(double)(1 +
							 ((first_burst_size > 0) ? first_burst_size : 0));
			} else {
				rtt_latency =
					((double)1.0 / (trans_completed / rtt_elapsed_time)) *
					(double)1000000.0;
		...
		if (local_cpu_usage) {
			...
			local_service_demand =
				calc_service_demand_fmt(('x' ==
										 libfmt) ? (double)trans_completed :
										bytes_xferd, 0.0, 0.0, 0);
		} else {
...
/* at some point we may want to actually display some results :) */
	...
	if (keep_statistics) {
		HIST_get_stats(time_hist,
					   &min_latency,
					   &max_latency, &mean_latency, &stddev_latency);
...
static void
set_hostname_and_port_2(void *addr, char *hostname, char *portstr, int family,
						int port)
{
...
void recv_omni()
{
	...
	strncpy(local_cong_control_req,
			omni_request->cong_control, sizeof(local_cong_control_req));
	...
	strncpy(local_fill_file, omni_request->fill_file, sizeof(local_fill_file));
	...
	switch (nf_to_af(omni_request->ipfamily)) {
		...
		default:
			for (ret = 0; ret < 4; ret++) {
				omni_request->netserver_ip[ret] =
					htonl(omni_request->netserver_ip[ret]);
				omni_request->netperf_ip[ret] =
					htonl(omni_request->netperf_ip[ret]);
			}
			break;
	}
	...
	if (debug) {
		fprintf(where,
				...
				omni_request->send_alignment, omni_request->send_offset);
		fflush(where);
	}
	...
	if (omni_request->direction & NETPERF_XMIT) {
		...
		if (omni_request->response_size > 0) {
			...
			if (omni_request->send_size == -1) {
				bytes_to_send =
					choose_send_size(lss_size, omni_request->protocol);
			} else
				bytes_to_send = omni_request->send_size;
			/* set the send_width */
			if (omni_request->send_width == 0) {
	...
	if (need_to_accept) {
		...
		if (use_fastopen &&
			(setsockopt
			 (s_listen, IPPROTO_TCP, TCP_FASTOPEN, &backlog,
			  sizeof(backlog)) == SOCKET_ERROR)) {
			netperf_response.content.serv_errno = errno;
			close(s_listen);
			send_response();
			if (debug) {
	...
	find_system_info(&local_system_model, &local_cpu_model,
					 &local_cpu_frequency);
	strncpy(omni_response->system_model, local_system_model,
			sizeof(omni_response->system_model));
	omni_response->system_model[sizeof(omni_response->system_model) - 1] = 0;
	strncpy(omni_response->cpu_model, local_cpu_model,
	...
	strncpy(omni_response->security_string,
			local_security_specific, sizeof(omni_response->security_string));
	omni_response->security_string[sizeof(omni_response->security_string) - 1] =
		0;
	...
	while ((!times_up) || (units_remaining > 0)) {
		...
		if (need_to_accept) {
			...
			if ((omni_request->protocol == IPPROTO_UDP) && (!peeraddr_set)) {
				peeraddr_set = 1;
		...
		if ((omni_request->direction & NETPERF_XMIT) &&
			...
			if (check_interval &&
				...
				if (get_unsent_data(data_socket, omni_request->protocol) > 0) {
					...
					if (check_interval == 0)
						check_interval = 1;
					while (get_unsent_data(data_socket, omni_request->protocol)
						   > 0) {
						/* presumably we could have given-up the CPU during the
						   ioctl() call in get_unsent_data() but might as well do
						   this too. raj 2012-07-31 */
			...
			ret = send_data(data_socket,
							...
							bytes_to_send,
							(connected) ? NULL : (struct sockaddr *)
							&peeraddr_in, addrlen, omni_request->protocol);
	...
	if (connected) {
		...
		if (omni_request->flags & OMNI_WANT_REM_CONG) {
			...
			strncpy(omni_results->cong_control, "",
					sizeof(omni_results->cong_control));
		}
	...
	omni_results->bytes_received_hi =
		(bytes_received & 0xFFFFFFFF00000000ULL) >> 32;
	...
	if (debug) {
		fprintf(where, "%s: test complete, sending results.\n", __FUNCTION__);
		fflush(where);
	}
...
%6d\n";
	...
	if (legacy) {
		...
		if (verbosity > 1) {
			...
			fprintf(where, ksink_fmt2, transport_mss);
#ifdef WANT_HISTOGRAM
		...
		if (verbosity > 1) {
			...
			fprintf(where,
					...
					(double)(1 +
							 ((first_burst_size > 0) ? first_burst_size : 0)),
					transaction_rate,
					calc_thruput_interval_omni(transaction_rate *
											   (double)req_size, 1.0),
					calc_thruput_interval_omni(transaction_rate *
											   (double)rsp_size, 1.0));
...
%5d  %5d   %5d  %5d\n";
	...
	send_omni_inner(remote_host,
					legacy, "MIGRATED TCP Connect/Request/Response TEST");
	...
	if (legacy) {
			...
			switch (verbosity) {
				case 0:
					if (local_cpu_usage) {
						fprintf(where,
								cpu_fmt_0,
								local_service_demand, local_cpu_method);
					} else {
						fprintf(where,
								cpu_fmt_0,
								remote_service_demand, remote_cpu_method);
					}
					break;
				case 1:
				case 2:
					if (print_headers) {
						fprintf(where,
								cpu_title, local_cpu_method, remote_cpu_method);
					}
		...
		if (verbosity > 1) {
			...
			fprintf(where,
					ksink_fmt,
					local_send_align,
					remote_recv_align, local_send_offset, remote_recv_offset);
...
%6d           %-7.2f   %7" PRIu64 "           %7.1f     %-6.2f   %-6.3f\n\n";
	...
	if (legacy) {
		...
		if (local_cpu_usage || remote_cpu_usage) {
			...
			switch (verbosity) {
				case 0:
					if (local_cpu_usage) {
						...
						fprintf(where,
								cpu_fmt_0,
								remote_service_demand, local_cpu_method);
					}
					break;
				case 1:
				case 2:
					...
					fprintf(where, cpu_fmt_1,	/* the format string */
							...
							send_size,	/* how large were the sends */
							elapsed_time,	/* how long was the test */
							local_send_calls, failed_sends, local_send_thruput,	/* what was the xfer rate */
							local_cpu_utilization,	/* local cpu */
							local_service_demand,	/* local service demand */
							rsr_size, elapsed_time, remote_receive_calls, remote_recv_thruput, remote_cpu_utilization,	/* remote cpu */
							remote_service_demand);	/* remote service demand */
					break;
			}
		} else {
			...
			switch (verbosity) {
				case 0:
					fprintf(where, tput_fmt_0, local_send_thruput);
					break;
				case 1:
				case 2:
					if (print_headers) {
					...
					fprintf(where, tput_fmt_1,	/* the format string */
							...
							send_size,	/* how large were the sends */
							elapsed_time,	/* how long did it take */
							local_send_calls, failed_sends, local_send_thruput, rsr_size,	/* remote recvbuf size */
							elapsed_time,
							remote_receive_calls, remote_recv_thruput);
					break;
			}
		}
...
static void set_omni_defaults_by_legacy_testname()
{
...
#define OMNI_ARGS "Bb:cCd:De:FgG:hH:i:IkK:l:L:m:M:nNoOp:P:r:R:s:S:t:T:u:Vw:W:46"
	...
	while ((c = getopt(argc, argv, OMNI_ARGS)) != EOF) {
		switch (c) {
...
#else
				printf
					("WARNING: TCP FASTOPEN not available on this platform!\n");
#endif
				break;
			case 'e':
			...
			case 'K':
				...
				if (arg1[0])
					strncpy(local_cong_control_req, arg1,
							sizeof(local_cong_control_req));
				if (arg2[2])
					strncpy(remote_cong_control_req, arg2,
	...
	if ((0 == direction) && (!connection_test))
		direction = NETPERF_XMIT;
	...
	if ((!have_R_option) &&
		(protocol == IPPROTO_UDP) && (!NETPERF_IS_RR(direction))) {
		routing_allowed = 0;
	}

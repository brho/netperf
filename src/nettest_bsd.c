...
char *protocol_to_str(int protocol)
{
	switch (protocol) {
...
static
void get_tcp_info(SOCKET socket, int *mss)
{
...
static
void set_tcp_mss(SOCKET socket, int mss)
{
#ifdef TCP_MAXSEG
	netperf_socklen_t sock_opt_len;
...
struct addrinfo *complete_addrinfo(char *controlhost, char *data_address,
								   char *port, int family, int type,
...
#define CHANGED_DCCP_SOCK  0x10
	...
	if (debug) {
		fprintf(where,
				...
				hostname,
				port,
				inet_ftos(family), inet_ttos(type), inet_ptos(protocol), flags);
		fflush(where);
	}
	...
	count = 0;
	do {
		error = getaddrinfo((char *)hostname, (char *)port, &hints, &res);
		count += 1;
		if (error == EAI_AGAIN) {
			if (debug) {
		...
		if (EAI_SOCKTYPE == error
			...
			if ((hints.ai_socktype == SOCK_DCCP) && (hints.ai_protocol == 0)) {
	...
	if (error) {
		fprintf(where,
				...
				hostname, port, family, error, gai_strerror(error));
		fflush(where);
		exit(-1);
	}
	...
	while (temp_res) {
		...
		if ((type) && (temp_res->ai_socktype != type)) {
		...
		if ((protocol) && (temp_res->ai_protocol != protocol)) {
...
void
complete_addrinfos(struct addrinfo **remote, struct addrinfo **local,
				   char remote_host[], int type, int protocol, int flags)
{
	...
	*remote = complete_addrinfo(remote_host,
								remote_data_address,
								remote_data_port,
								remote_data_family, type, protocol, flags);
	...
	*local = complete_addrinfo("what to put here?",
							   ...
							   local_data_port,
							   local_data_family,
							   type, protocol, flags | AI_PASSIVE);
...
/* stuff the address family, port number and address into a
   ...
   first */
void
set_sockaddr_family_addr_port(struct sockaddr_storage *sockaddr, int family,
							  void *addr, int port)
{
...
int
get_sockaddr_family_addr_port(struct sockaddr_storage *sockaddr, int family,
							  void *addr, int *port)
{
	...
	switch (family) {
		...
		case AF_INET:
			...
			if (setsockopt(sock,
				...
				fprintf(where, "%s ip_tos failed with %s (errno %d)\n",
						__FUNCTION__, strerror(errno), errno);
				fflush(where);
				my_tos = -2;
			} else {
				...
				getsockopt(sock,
						   IPPROTO_IP, IP_TOS, (char *)&my_tos, &sock_opt_len);
			}
			break;
#endif
		...
		case AF_INET6:
			...
			if (setsockopt(sock,
				...
				getsockopt(sock,
						   IPPROTO_IPV6,
						   IPV6_TCLASS, (char *)&my_tos, &sock_opt_len);
			}
			break;
#endif
	}
...
SOCKET create_data_socket(struct addrinfo * res)
{
	...
	if (temp_socket == INVALID_SOCKET) {
		fprintf(where,
				...
				inet_ptos(res->ai_protocol), strerror(errno));
		fflush(where);
		exit(1);
	}
	...
	if ((loc_nodelay) && (res->ai_protocol != IPPROTO_UDP)) {
		...
		if (setsockopt(temp_socket,
					   res->ai_protocol,
					   option, (char *)&one, sizeof(one)) == SOCKET_ERROR) {
			fprintf(where,
	...
	if (want_keepalive) {
		if (setsockopt(temp_socket,
					   SOL_SOCKET,
					   SO_KEEPALIVE, (const char *)&on, sizeof(on)) < 0) {
			if (debug) {
				fprintf(where,
...
#undef NETPERF_NEED_CLEANUP
				   ...
				   SOL_SOCKET,
#endif
				   SO_REUSEADDR, (const char *)&on, sizeof(on)) < 0) {
		fprintf(where,
				"netperf: create_data_socket: SO_REUSEADDR failed %d\n", errno);
		fflush(where);
	}

	if (bind(temp_socket, res->ai_addr, res->ai_addrlen) < 0) {
		if (debug) {
			fprintf(where,
	...
	if (local_socket_prio >= 0) {
		if (setsockopt(temp_socket,
					   SOL_SOCKET,
					   SO_PRIORITY,
					   &local_socket_prio, sizeof(int)) == SOCKET_ERROR) {
			fprintf(where,
			...
			getsockopt(temp_socket,
					   SOL_SOCKET,
					   SO_PRIORITY, &local_socket_prio, &sock_opt_len);
		}
	}
#else
	local_socket_prio = -3;
	...
	if (local_socket_tos > 0)
		local_socket_tos =
			set_socket_tos(temp_socket, res->ai_family, local_socket_tos);
#endif
...
void kludge_socket_options(int temp_socket)
{
...
void
print_top_test_header(char test_name[], struct addrinfo *source,
					  struct addrinfo *destination)
{
	...
	address_buf[0] = '\0';
	inet_ntop(source->ai_family, get_address_address(source), address_buf,
			  sizeof(address_buf));
	fprintf(where, " from %s (%s) port %u %s", source->ai_canonname,
			address_buf, get_port_number(source), inet_ftos(source->ai_family));

	address_buf[0] = '\0';
	inet_ntop(destination->ai_family, get_address_address(destination),
			  address_buf, sizeof(address_buf));
	fprintf(where, " to %s (%s) port %u %s", destination->ai_canonname,
			address_buf, get_port_number(destination),
	...
	if ((loc_sndavoid > 0) ||
		(loc_rcvavoid > 0) || (rem_sndavoid > 0) || (rem_rcvavoid > 0)) {
...
%6d\n";
	...
	tcp_stream_request =
		(struct tcp_stream_request_struct *)netperf_request.content.
		test_specific_data;
	tcp_stream_response =
		(struct tcp_stream_response_struct *)netperf_response.content.
		test_specific_data;
	tcp_stream_result =
		(struct tcp_stream_results_struct *)netperf_response.content.
		test_specific_data;
	...
	complete_addrinfos(&remote_res,
					   &local_res, remote_host, SOCK_STREAM, IPPROTO_TCP, 0);
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		while ((!times_up) || (bytes_remaining > 0)) {
			...
			access_buffer(send_ring->buffer_ptr,
						  send_size, loc_dirty_count, loc_clean_count);
#endif /* DIRTY */
...
%6d\n";
	...
	tcp_maerts_request =
		(struct tcp_maerts_request_struct *)netperf_request.content.
		test_specific_data;
	tcp_maerts_response =
		(struct tcp_maerts_response_struct *)netperf_response.content.
		test_specific_data;
	tcp_maerts_result =
		(struct tcp_maerts_results_struct *)netperf_response.content.
		test_specific_data;
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		while ((!times_up) && (len = recv(recv_socket,
			...
			access_buffer(recv_ring->buffer_ptr,
						  recv_size, loc_dirty_count, loc_clean_count);
#endif /* DIRTY */
...
Size (bytes)\n\n";
	...
	switch (verbosity) {
		...
		case 2:
			...
			fprintf(where, mss_fmt_0,	/* the format string */
					tcp_mss,
					((print_headers) ||
					 (result_brand == NULL)) ? "" : result_brand);
			break;
...
void send_exs_tcp_stream(char remote_host[])
{
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		if (debug) {
			fprintf(where, "send_exs_tcp_stream: "
					...
					nummessages, exs_aio_eagain, exs_aio_dequeuecnt,
					exs_aio_pending);
		}
	...
	if (local_cpu_usage || remote_cpu_usage) {
		...
		switch (verbosity) {
			case 0:
				if (local_cpu_usage) {
					fprintf(where,
							cpu_fmt_0, local_service_demand, local_cpu_method);
				} else {
					fprintf(where,
							cpu_fmt_0,
...
void sendfile_tcp_stream(remote_host)
char remote_host[];
{
...
#define REQUEST_CWND_INITIAL 2
	...
	tcp_rr_request =
		(struct tcp_rr_request_struct *)netperf_request.content.
		test_specific_data;
	tcp_rr_response =
		(struct tcp_rr_response_struct *)netperf_response.content.
		test_specific_data;
	tcp_rr_result =
		(struct tcp_rr_results_struct *)netperf_response.content.
		test_specific_data;
	...
	if (print_headers) {
		print_top_test_header("TCP REQUEST/RESPONSE TEST", local_res,
							  remote_res);
	}
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		if (!no_control) {
			...
			if (!netperf_response.content.serv_errno) {
				...
				set_port_number(remote_res,
								(short)tcp_rr_response->data_port_number);
			} else {
				Set_errno(netperf_response.content.serv_errno);
				fprintf(where,
						"netperf: remote error %d",
			...
			if (request_cwnd < first_burst_size) {
				...
				if (debug) {
					fprintf(where,
							...
							request_cwnd,
							first_burst_size, requests_outstanding);
				}
			}
#endif
			if (timed_out) {
		...
		if (local_cpu_usage || remote_cpu_usage) {
			...
			if (local_cpu_usage) {
				...
				/* since calc_service demand is doing ms/Kunit we will
				   ...
				   numbers */
				local_service_demand =
					calc_service_demand((double)nummessages * 1024, 0.0, 0.0,
										0);
			} else {
				local_cpu_utilization = (float)-1.0;
				local_service_demand = (float)-1.0;
		...
		switch (verbosity) {
			...
			case 2:
				...
				fprintf(where, cpu_fmt_1_line_1,	/* the format string */
						...
						rsp_size,	/* guess */
						elapsed_time,	/* how long was the test */
						('x' == libfmt) ? thruput : calc_thruput_interval_omni(thruput * (req_size + rsp_size), 1.0), local_cpu_utilization,	/* local cpu */
						remote_cpu_utilization,	/* remote cpu */
						local_service_demand,	/* local service demand */
						remote_service_demand,	/* remote service demand */
						((print_headers) ||
		...
		switch (verbosity) {
			case 0:
				fprintf(where,
						tput_fmt_0,
						('x' == libfmt) ? thruput :
						calc_thruput_interval_omni(thruput *
												   (req_size + rsp_size), 1.0),
						((print_headers)
						 || (result_brand == NULL)) ? "" : result_brand);
				break;
			case 1:
...
%6d           %-7.2f   %7d           %7.1f     %-6.2f   %-6.3f\n\n";
	...
	udp_stream_request =
		(struct udp_stream_request_struct *)netperf_request.content.
		test_specific_data;
	udp_stream_response =
		(struct udp_stream_response_struct *)netperf_response.content.
		test_specific_data;
	udp_stream_results =
		(struct udp_stream_results_struct *)netperf_response.content.
		test_specific_data;
	...
	complete_addrinfos(&remote_res,
					   &local_res, remote_host, SOCK_DGRAM, IPPROTO_UDP, 0);

	if (print_headers) {
		print_top_test_header("UDP UNIDIRECTIONAL SEND TEST", local_res,
							  remote_res);
	}
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		if (send_size == 0) {
			if (lss_size > 0) {
				send_size =
					(lss_size < UDP_LENGTH_MAX ? lss_size : UDP_LENGTH_MAX);
			} else {
				send_size = 4096;
			}
		}
		...
		if (!no_control) {
			...
			set_port_number(remote_res,
							(short)udp_stream_response->data_port_number);
		...
		while (!times_up) {
			...
			if (local_connected) {
				len = send(data_socket, send_ring->buffer_ptr, send_size, 0);
			} else {
				len = sendto(data_socket,
							 send_ring->buffer_ptr,
							 send_size,
							 0, remote_res->ai_addr, remote_res->ai_addrlen);
			}
		...
		switch (verbosity) {
			case 0:
				if (local_cpu_usage) {
					...
					fprintf(where,
							cpu_fmt_0, remote_service_demand, local_cpu_method);
				}
				break;
			case 1:
			case 2:
				...
				fprintf(where, cpu_fmt_1,	/* the format string */
						...
						send_size,	/* how large were the sends */
						elapsed_time,	/* how long was the test */
						messages_sent, failed_sends, local_thruput,	/* what was the xfer rate */
						local_cpu_utilization,	/* local cpu */
						local_service_demand,	/* local service demand */
						rsr_size, elapsed_time, messages_recvd, remote_thruput, remote_cpu_utilization,	/* remote cpu */
						remote_service_demand);	/* remote service demand */
				break;
		}
	} else {
		...
		switch (verbosity) {
			...
			case 2:
				...
				fprintf(where, tput_fmt_1,	/* the format string */
						...
						send_size,	/* how large were the sends */
						elapsed_time,	/* how long did it take */
						messages_sent, failed_sends, local_thruput, rsr_size,	/* remote recvbuf size */
...
void recv_udp_stream()
{
	...
	while (!times_up) {
		if (local_connected) {
			len = recv(s_data, recv_ring->buffer_ptr, message_size, 0);
		} else {
			len = recvfrom(s_data,
						   recv_ring->buffer_ptr, message_size, 0, 0, 0);
		}
...
%-6d %-6d\n";
	...
	udp_rr_request =
		(struct udp_rr_request_struct *)netperf_request.content.
		test_specific_data;
	udp_rr_response =
		(struct udp_rr_response_struct *)netperf_response.content.
		test_specific_data;
	udp_rr_result =
		(struct udp_rr_results_struct *)netperf_response.content.
		test_specific_data;
	...
	if (print_headers) {
		print_top_test_header("UDP REQUEST/RESPONSE TEST", local_res,
							  remote_res);
	}
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		if (!no_control) {
			...
			if (!netperf_response.content.serv_errno) {
				...
				set_port_number(remote_res,
								(short)udp_rr_response->data_port_number);
			} else {
				Set_errno(netperf_response.content.serv_errno);
				fprintf(where,
						"netperf: remote error %d",
...
void recv_udp_rr()
{
	...
	if (udp_rr_request->measure_cpu) {
		...
		udp_rr_response->cpu_rate =
			calibrate_local_cpu(udp_rr_request->cpu_rate);
	}
...
void recv_tcp_rr()
{
	...
	if (tcp_rr_request->measure_cpu) {
		...
		tcp_rr_response->cpu_rate =
			calibrate_local_cpu(tcp_rr_request->cpu_rate);
	}
...
void loc_cpu_rate()
{
	...
	if (verbosity > 1)
		fprintf(where,
				...
				lib_num_loc_cpus, (lib_num_loc_cpus > 1) ? "cpus" : "cpu");
...
void rem_cpu_rate()
{
	...
	if (verbosity > 1)
		fprintf(where,
				...
				lib_num_rem_cpus, (lib_num_rem_cpus > 1) ? "cpus" : "cpu");
...
%5d  %5d   %5d  %5d\n";
	...
	tcp_conn_rr_request =
		(struct tcp_conn_rr_request_struct *)netperf_request.content.
		test_specific_data;
	tcp_conn_rr_response =
		(struct tcp_conn_rr_response_struct *)netperf_response.content.
		test_specific_data;
	tcp_conn_rr_result =
		(struct tcp_conn_rr_results_struct *)netperf_response.content.
		test_specific_data;
	...
	if (print_headers) {
		print_top_test_header("TCP Connect/Request/Response TEST", local_res,
							  remote_res);
	}
	...
	if (!no_control) {
		...
		if (!netperf_response.content.serv_errno) {
			...
			/* make sure that port numbers are in network order */
			set_port_number(remote_res,
							(unsigned short)tcp_conn_rr_response->
							data_port_number);
...
newport:
		...
		if ((ret = connect(send_socket,
			...
			printf("\tattempted to connect on socket %d to port %d",
				   send_socket, get_port_number(remote_res));
		...
		do {
			rsp_bytes_recvd = recv(send_socket,
								   temp_message_ptr, rsp_bytes_left, 0);
		...
		rsp_bytes_recvd = recv(send_socket, temp_message_ptr, 1, 0);
		...
		if (rsp_bytes_recvd == 0) {
			...
			if (debug > 3) {
				fprintf(where,
						...
						nummessages, get_port_number(local_res));
				fflush(where);
			}
		...
		switch (verbosity) {
			...
			case 2:
				...
				fprintf(where, cpu_fmt_1_line_1,	/* the format string */
						...
						rsp_size,	/* guess */
						elapsed_time,	/* how long was the test */
						nummessages / elapsed_time, local_cpu_utilization,	/* local cpu */
						remote_cpu_utilization,	/* remote cpu */
						local_service_demand,	/* local service demand */
						remote_service_demand);	/* remote service demand */
				fprintf(where, cpu_fmt_1_line_2, rss_size, rsr_size);
...
int send_tcp_tran_rr(char remote_host[])
{
...
%5d  %5d   %5d  %5d\n";
	...
	tcp_tran_rr_request =
		(struct tcp_tran_rr_request_struct *)netperf_request.content.
		test_specific_data;
	tcp_tran_rr_response =
		(struct tcp_tran_rr_response_struct *)netperf_response.content.
		test_specific_data;
	tcp_tran_rr_result =
		(struct tcp_tran_rr_results_struct *)netperf_response.content.
		test_specific_data;
	...
	bzero((char *)&server, sizeof(server));
	bzero((char *)myaddr, sizeof(struct sockaddr_storage));
	...
	if (print_headers) {
		print_top_test_header("TCP Transactional/Request/Response TEST",
							  local_res, remote_res);
	}
...
int recv_tcp_tran_rr()
{
	...
	if (debug) {
		fprintf(where,
				"recv_tcp_tran_rr: receive alignment and offset set...\n");
		fflush(where);
	}
	...
	if (setsockopt(s_listen,
				   IPPROTO_TCP,
				   TCP_NOPUSH,
				   (const char *)&NoPush, sizeof(int)) == SOCKET_ERROR) {
		fprintf(where,
...
void send_tcp_nbrr(char remote_host[])
{
...
%5d  %5d   %5d  %5d\n";
	...
	if (print_headers) {
		print_top_test_header("TCP Non-Blocking REQUEST/RESPONSE TEST",
							  local_res, remote_res);
	}
...
void recv_tcp_nbrr()
{
	...
	if (debug) {
		fprintf(where,
				"recv_tcp_nbrr: requested recv alignment of %d offset %d\n",
				tcp_rr_request->recv_alignment, tcp_rr_request->recv_offset);
		fprintf(where,
				"recv_tcp_nbrr: requested send alignment of %d offset %d\n",
				tcp_rr_request->send_alignment, tcp_rr_request->send_offset);
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where,
				"recv_tcp_nbrr: accept completes on the data connection.\n");
		fflush(where);
	}
#ifdef KLUDGE_SOCKET_OPTIONS
	/* this is for those systems which *INCORRECTLY* fail to pass */
...
%5d  %5d   %5d  %5d\n";
	...
	tcp_cc_request =
		(struct tcp_cc_request_struct *)netperf_request.content.
		test_specific_data;
	tcp_cc_response =
		(struct tcp_cc_response_struct *)netperf_response.content.
		test_specific_data;
	tcp_cc_result =
		(struct tcp_cc_results_struct *)netperf_response.content.
		test_specific_data;
	...
	if (!netperf_response.content.serv_errno) {
		...
		/* make sure that port numbers are in network order */
		set_port_number(remote_res,
						(unsigned short)tcp_cc_response->data_port_number);
...
void recv_tcp_cc()
{
	...
	recv_message_ptr =
		ALIGN_BUFFER(message, tcp_cc_request->recv_alignment,
					 tcp_cc_request->recv_offset);

	send_message_ptr =
		ALIGN_BUFFER(message, tcp_cc_request->send_alignment,
...
#define SOCKETS_ARGS "b:CDnNhH:L:m:M:p:P:r:R:s:S:T:Vw:W:z46"
	...
	if (no_control) {
		...
		if (strcmp(remote_data_port, "0") == 0) {
			...
			if (strstr(test_name, "STREAM") || strstr(test_name, "SENDFILE")) {

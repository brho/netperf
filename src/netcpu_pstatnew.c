...
void get_cpu_counters(cpu_time_counters_t * res)
{
	...
	if (pstat_getprocessor(psp, sizeof(*psp), max_proc_count, 0) != -1) {
		...
		while ((i < lib_num_loc_cpus) && (j < max_proc_count)) {
			...
			if (debug) {
				fprintf(where, "\tidle[%d] = 0x%" PRIx64 " ", i, res[i].idle);
				fflush(where);
			}
			res[i].user = (((uint64_t) psp[j].psp_usercycles.psc_hi << 32) +
						   psp[j].psp_usercycles.psc_lo);
			if (debug) {
				fprintf(where, "user[%d] = 0x%" PRIx64 " ", i, res[i].user);
				fflush(where);
			}
			res[i].kernel = (((uint64_t) psp[j].psp_systemcycles.psc_hi << 32) +
							 psp[j].psp_systemcycles.psc_lo);
			if (debug) {
				fprintf(where, "kern[%d] = 0x%" PRIx64 " ", i, res[i].kernel);
				fflush(where);
			}
			res[i].interrupt =
				(((uint64_t) psp[j].psp_interruptcycles.psc_hi << 32) +
...
static void
print_cpu_time_counters(char *name, int instance,
						cpu_time_counters_t * counters)
{
	fprintf(where,
			"%s[%d]:\n"
			"\t idle %llu\n"
			...
			counters[instance].kernel, counters[instance].interrupt);
}
...
#define CALC_ACCURACY CALC_THOUSANDTH_PERCENT
	...
	if (debug) {
		fprintf(where,
				...
				"\n", lib_elapsed, sysconf(_SC_CLK_TCK), lib_iticksperclktick);
	}
	...
	for (i = 0; i < lib_num_loc_cpus; i++) {
		...
		if (debug) {
			print_cpu_time_counters("delta_cpu_counters", i,
									delta_cpu_counters);
		}
		...
		total_cpu_cycles =
			...
			delta_cpu_counters[i].kernel + delta_cpu_counters[i].interrupt;
		...
		if (debug) {
			fprintf(where,
					...
					" missing %" PRIu64 "\n",
					total_cpu_cycles,
					sanity_cpu_cycles, sanity_cpu_cycles - total_cpu_cycles);
		}
		...
		estimated_fraction_interrupt =
			((double)delta_cpu_counters[i].interrupt +
			 (sanity_cpu_cycles -
			  total_cpu_cycles)) / (double)sanity_cpu_cycles;
		...
		if (debug) {
			fprintf(where,
					...
					fraction_user,
					fraction_kernel,
					fraction_interrupt, estimated_fraction_interrupt);
		}
		...
		fraction_interrupt =
			(delta_cpu_counters[i].interrupt * CALC_ACCURACY) /
			sanity_cpu_cycles;
		...
		if (debug) {
			fprintf(where,
					...
					"\tfraction_interrupt %" PRIu64
					"WARNING, possibly under-counted!\n"
					"\testimated_fraction_interrupt %" PRIu64 "\n",
					fraction_idle, fraction_user, fraction_kernel,
					fraction_interrupt, estimated_fraction_interrupt);
		}
		...
		if (debug) {
			fprintf(where,
					...
					i, lib_local_per_cpu_util[i], correction_factor);
		}
	...
	if (debug) {
		fprintf(where, "calc_cpu_util: returning %g\n", lib_local_cpu_util);
	}

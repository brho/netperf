...
#ifdef NOTDEF
static void bind_to_processor(int child_num)
{
...
#include <sys/procset.h>
	...
	if (processor_bind(P_PID,
					   getpid(),
					   (child_num % lib_num_loc_cpus), &old_binding) != 0) {
		fprintf(where,
				"bind_to_processor: unable to perform processor binding\n");
	...
	if (!SetThreadAffinityMask
		(GetCurrentThread(), (ULONG_PTR) 1 << (child_num % lib_num_loc_cpus))) {
	...
	if (debug) {
		fprintf(where,
				...
				child_num, (child_num % lib_num_loc_cpus), lib_num_loc_cpus);
		fflush(where);
	}
#endif
	return;
...
static void sit_and_spin(int child_index)
{
	...
	for (*my_counter_ptr = 0L;; (*my_counter_ptr)++) {
...
static void start_looper_processes()
{
	...
	file_size = ((netlib_get_page_size() * PAGES_PER_CHILD) * lib_num_loc_cpus);
...
#define MAP_FILE 0x0000
	...
	if (debug > 1) {
		fprintf(where, "num CPUs %d, file_size %d, lib_base_pointer %p\n",
				lib_num_loc_cpus, file_size, lib_base_pointer);
		fflush(where);
	}
	...
	for (i = 0; i < (unsigned int)lib_num_loc_cpus; i++) {
		...
		if ((lib_idle_pids[i] = CreateThread(0,
											 ...
											 sit_and_spin,
											 (LPVOID) (ULONG_PTR) i, 0,
											 &place_holder)) == NULL) {
			fprintf(where, "create_looper_process: CreateThread failed\n");
			fflush(where);
...
void cpu_util_terminate()
{
	...
	munmap((caddr_t) lib_base_pointer,
		   ((netlib_get_page_size() * PAGES_PER_CHILD) * lib_num_loc_cpus));
...
float calibrate_idle_rate(int iterations, int interval)
{
	...
	uint64_t firstcnt[MAXCPUS], secondcnt[MAXCPUS];

	float elapsed, temp_rate, rate[MAXTIMES], local_maxrate;

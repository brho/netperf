...
int put_control(fd, len, pri, ack)
int fd, len, pri, ack;
{
	int error;
	int flags = 0;
...
int dl_open(char devfile[], int ppa)
{
	int fd;
...
int dl_bind(int fd, int sap, int mode, char *dlsap_ptr, int *dlsap_len)
{
	...
	if (*dlsap_len >= bind_ack->dl_addr_length) {
		bcopy((char *)bind_ack + bind_ack->dl_addr_offset,
			  dlsap_ptr, bind_ack->dl_addr_length);
...
int dl_connect(int fd, unsigned char *remote_addr, int remote_addr_len)
{
	...
	while (control_data[0] == DL_TEST_CON) {
		...
		if ((error = getmsg(fd, &control_message, &data_message, &flags)) != 0) {
			fprintf(where,
					"dl_connect: getmsg failure, errno = %d, error = 0x%x\n",
					errno, error);
			fflush(where);
			return (-1);
		}
...
int dl_accept(fd, remote_addr, remote_addr_len)
int fd;
...
%5d   %5d  %5d   %5d %6.4g  %6.2f     %6d %6.2f   %6d\n";
	...
	dlpi_co_stream_request =
		(struct dlpi_co_stream_request_struct *)netperf_request.content.
		test_specific_data;
	dlpi_co_stream_response =
		(struct dlpi_co_stream_response_struct *)netperf_response.content.
		test_specific_data;
	dlpi_co_stream_result =
		(struct dlpi_co_stream_results_struct *)netperf_response.content.
		test_specific_data;
	...
	dlsap_len = BUFSIZ;
	if (dl_bind(send_descriptor, dlpi_sap, DL_CODLS, dlsap, &dlsap_len) != 0) {
	...
	if (lsw_size > 0) {
		if (debug > 1) {
			fprintf(where,
					"netperf: send_dlpi_co_stream: window send size altered from system default...\n");
	...
	strcpy(dlpi_co_stream_request->dlpi_device, rem_dlpi_device);
	...
	while ((!times_up) || (bytes_remaining > 0)) {
		...
		if ((putmsg(send_descriptor, 0, &send_message, 0)) != 0) {
			if (errno == EINTR)
				break;
		...
		for (interval_count = 0;
			 interval_count < interval_wate; interval_count++) ;
#endif /* WANT_INTERVALS */
...
int recv_dlpi_co_stream()
{
	...
	message_ptr =
		ALIGN_BUFFER(message, dlpi_co_stream_request->recv_alignment,
	...
	while (recv_message.len == recv_size) {
		if (getmsg(data_descriptor, 0, &recv_message, &flags) != 0) {
			netperf_response.content.serv_errno = errno;
			send_response();
			exit(1);
		}
	...
	if (debug) {
		fprintf(where, "recv_dlpi_co_stream: got %d bytes\n", bytes_received);
		fprintf(where, "recv_dlpi_co_stream: got %d recvs\n", receive_calls);
		fflush(where);
	}
...
int send_dlpi_co_rr(char remote_host[])
{
 ...
 %5d  %5d   %5d  %5d\n";
	...
	dlpi_co_rr_request =
		(struct dlpi_co_rr_request_struct *)netperf_request.content.
		test_specific_data;
	dlpi_co_rr_response =
		(struct dlpi_co_rr_response_struct *)netperf_response.content.
		test_specific_data;
	dlpi_co_rr_result =
		(struct dlpi_co_rr_results_struct *)netperf_response.content.
		test_specific_data;
	...
	if (lsw_size > 0) {
		if (debug > 1) {
			fprintf(where,
					"netperf: send_dlpi_co_rr: socket send size altered from system default...\n");
			fprintf(where, "                          send: %d\n", lsw_size);
		}
	}
	if (lrw_size > 0) {
		if (debug > 1) {
			fprintf(where,
					"netperf: send_dlpi_co_rr: socket recv size altered from system default...\n");
			fprintf(where, "                          recv: %d\n", lrw_size);
		}
	}
	...
	if (debug) {
		fprintf(where,
				"netperf: send_dlpi_co_rr: socket sizes determined...\n");
		fprintf(where, "         send: %d recv: %d\n", lsw_size, lrw_size);
	}
#else /* DL_HP_SET_LOCAL_WIN_REQ */
	...
	strcpy(dlpi_co_rr_request->dlpi_device, rem_dlpi_device);
#ifdef __alpha
	...
	if (debug > 1) {
		fprintf(where,
				"netperf: send_dlpi_co_rr: requesting TCP stream test\n");
	}
	...
	while ((!times_up) || (trans_remaining > 0)) {
		...
		while (rsp_bytes_left > 0) {
			if ((getmsg(send_descriptor, 0, &recv_message, &flags)) < 0) {
				if (errno == EINTR) {
		...
		switch (verbosity) {
			...
			case 1:
				fprintf(where, cpu_fmt_1_line_1,	/* the format string */
						lsw_size,	/* local sendbuf size */
						lrw_size, req_size,	/* how large were the requests */
						rsp_size,	/* guess */
						elapsed_time,	/* how long was the test */
						...
						local_service_demand,	/* local service demand */
						remote_service_demand);	/* remote service demand */
				fprintf(where, cpu_fmt_1_line_2, rsw_size, rrw_size);
				break;
		}
	} else {
		/* The tester did not wish to measure service demand. */
		switch (verbosity) {
			...
			case 1:
				fprintf(where, tput_fmt_1_line_1,	/* the format string */
						lsw_size, lrw_size, req_size,	/* how large were the requests */
						rsp_size,	/* how large were the responses */
						elapsed_time,	/* how long did it take */
						nummessages / elapsed_time);
				fprintf(where, tput_fmt_1_line_2, rsw_size,	/* remote recvbuf size */
						rrw_size);
...

void send_dlpi_cl_stream(char remote_host[])
{
  /************************************************************************/
	/*                                    */
...
%5d            %-7.2f   %7d           %7.1f      %-6.2f  %-6.3f\n\n";
	...
	dlpi_cl_stream_request =
		(struct dlpi_cl_stream_request_struct *)netperf_request.content.
		test_specific_data;
	dlpi_cl_stream_response =
		(struct dlpi_cl_stream_response_struct *)netperf_response.content.
		test_specific_data;
	dlpi_cl_stream_results =
		(struct dlpi_cl_stream_results_struct *)netperf_response.content.
		test_specific_data;
	...
	/* bind the puppy and get the assigned dlsap */
	dlsap_len = BUFSIZ;
	if (dl_bind(data_descriptor, dlpi_sap, DL_CLDLS, dlsap, &dlsap_len) != 0) {
	...
	if (lsw_size > 0) {
		if (debug > 1) {
			fprintf(where,
					"netperf: send_dlpi_cl_stream: descriptor send size altered from system default...\n");
			fprintf(where, "                          send: %d\n", lsw_size);
		}
	}
	if (lrw_size > 0) {
		if (debug > 1) {
			fprintf(where,
					"netperf: send_dlpi_cl_stream: descriptor recv size altered from system default...\n");
			fprintf(where, "                          recv: %d\n", lrw_size);
		}
	}
	...
	if (send_width == 0)
		send_width = 32;
	message =
		(char *)malloc(send_size * (send_width + 1) + local_send_align +
					   local_send_offset);
	if (message == NULL) {
		printf("malloc(%d) failed!\n",
			   send_size * (send_width + 1) + local_send_align +
			   local_send_offset);
		exit(1);
	}
	...
	strcpy(dlpi_cl_stream_request->dlpi_device, rem_dlpi_device);
	...
	if (!netperf_response.content.serv_errno) {
		if (debug)
			fprintf(where,
					"send_dlpi_cl_stream: remote data connection done.\n");
	} else {
		Set_errno(netperf_response.content.serv_errno);
...
#endif /* DIRTY */
		if (putmsg(data_descriptor, &sctl_message, &send_message, 0) != 0) {
			if (errno == EINTR) {
				break;
			}
			if (errno == ENOBUFS) {
		...
		switch (verbosity) {
			...
			case 1:
				fprintf(where, cpu_fmt_1,	/* the format string */
						...
						local_cpu_utilization,	/* local cpu */
						local_service_demand,	/* local service demand */
						rrw_size, elapsed_time, messages_recvd, remote_thruput, remote_cpu_utilization,	/* remote cpu */
						remote_service_demand);	/* remote service demand */
				break;
		}
	} else {
		...
		switch (verbosity) {
			...
			case 1:
				fprintf(where, tput_fmt_1,	/* the format string */
						...
						send_size,	/* how large were the sends */
						elapsed_time,	/* how long did it take */
						messages_sent, failed_sends, local_thruput, rrw_size,	/* remote recvbuf size */
...
int recv_dlpi_cl_stream()
{
	...
	message_ptr =
		ALIGN_BUFFER(message, dlpi_cl_stream_request->recv_alignment,
					 dlpi_cl_stream_request->recv_offset);
	...
	if (dlpi_cl_stream_request->measure_cpu) {
		...
		dlpi_cl_stream_response->cpu_rate =
			calibrate_local_cpu(dlpi_cl_stream_request->cpu_rate);
	}
	...
	if (debug) {
		fprintf(where, "recv_dlpi_cl_stream: got %d bytes\n", bytes_received);
		fflush(where);
	}
...
#define	MAX_KEPT_TIMES	1024
	...
	dlpi_cl_rr_request =
		(struct dlpi_cl_rr_request_struct *)netperf_request.content.
		test_specific_data;
	dlpi_cl_rr_response =
		(struct dlpi_cl_rr_response_struct *)netperf_response.content.
		test_specific_data;
	dlpi_cl_rr_result =
		(struct dlpi_cl_rr_results_struct *)netperf_response.content.
		test_specific_data;
	...
	dlsap_len = BUFSIZ;
	if (dl_bind(send_descriptor, dlpi_sap, DL_CLDLS, dlsap, &dlsap_len) != 0) {
	...
	if (lsw_size > 0) {
		if (debug > 1) {
			fprintf(where,
					"netperf: send_dlpi_cl_rr: local window size altered from system default...\n");
	...
	if (lrw_size > 0) {
		if (debug > 1) {
			fprintf(where,
					"netperf: send_dlpi_cl_rr: remote window size altered from system default...\n");
	...
	if (debug) {
		fprintf(where,
				"netperf: send_dlpi_cl_rr: socket sizes determined...\n");
		fprintf(where, "         send: %d recv: %d\n", lsw_size, lrw_size);
	}
#else /* SO_SNDBUF */
	...
	strcpy(dlpi_cl_rr_request->dlpi_device, rem_dlpi_device);
	...
	if (debug > 1) {
		fprintf(where,
				"netperf: send_dlpi_cl_rr: requesting DLPI CL request/response test\n");
	}
...
#endif /* WANT_INTERVALS */
		if (putmsg(send_descriptor, &sctl_message, &send_message, 0) != 0) {
			if (errno == EINTR) {
				/* We likely hit */
				/* test-end time. */
				break;
	...
	if (verbosity > 1) {
		...
		fprintf(where, "Average response time %d usecs\n", total_times);
#endif
	}
}

int recv_dlpi_cl_rr()
{
	...
	recv_message_ptr =
		ALIGN_BUFFER(message, dlpi_cl_rr_request->recv_alignment,
					 dlpi_cl_rr_request->recv_offset);
	...
	send_message_ptr =
		ALIGN_BUFFER(message, dlpi_cl_rr_request->send_alignment,
					 dlpi_cl_rr_request->send_offset);
	...
	if (debug) {
		fprintf(where,
				"recv_dlpi_cl_rr: receive alignment and offset set...\n");
	...
	if (dlpi_cl_rr_request->measure_cpu) {
		...
		dlpi_cl_rr_response->cpu_rate =
			calibrate_local_cpu(dlpi_cl_rr_request->cpu_rate);
	}
	...
	if (debug) {
		fprintf(where, "recv_dlpi_cl_rr: test complete, sending results.\n");
		fflush(where);
	}
...
int recv_dlpi_co_rr()
{
	...
	recv_message_ptr =
		ALIGN_BUFFER(message, dlpi_co_rr_request->recv_alignment,
					 dlpi_co_rr_request->recv_offset);
	...
	send_message_ptr =
		ALIGN_BUFFER(message, dlpi_co_rr_request->send_alignment,
					 dlpi_co_rr_request->send_offset);
	...
	if (debug) {
		fprintf(where,
				"recv_dlpi_co_rr: receive alignment and offset set...\n");
		fprintf(where,
				"recv_dlpi_co_rr: send_message.buf %x .len %d .maxlen %d\n",
				send_message.buf, send_message.len, send_message.maxlen);
		fprintf(where,
				"recv_dlpi_co_rr: recv_message.buf %x .len %d .maxlen %d\n",
	...
	if (dlpi_co_rr_request->measure_cpu) {
		...
		dlpi_co_rr_response->cpu_rate =
			calibrate_local_cpu(dlpi_co_rr_request->cpu_rate);
	}
	...
	if (dl_accept(data_descriptor, 0, 0) != 0) {
		fprintf(where, "recv_dlpi_co_rr: error in accept, errno %d\n", errno);
		fflush(where);
		netperf_response.content.serv_errno = errno;
		send_response();
		exit(1);
	...
	while ((!times_up) || (trans_remaining > 0)) {
		...
		if ((putmsg(data_descriptor, 0, &send_message, 0)) != 0) {
			if (errno == EINTR) {
	...
	if (debug) {
		fprintf(where, "recv_dlpi_co_rr: test complete, sending results.\n");
		fflush(where);
	}

...
static
void get_sdp_info(int socket, int *mss)
{
...
void send_sdp_stream(char remote_host[])
{
...
%6d\n";
	...
	sdp_stream_request =
		(struct sdp_stream_request_struct *)netperf_request.content.
		test_specific_data;
	sdp_stream_response =
		(struct sdp_stream_response_struct *)netperf_response.content.
		test_specific_data;
	sdp_stream_result =
		(struct sdp_stream_results_struct *)netperf_response.content.
		test_specific_data;
	...
	if (verbosity > 1) {
		...
		fprintf(where, ksink_fmt2, sdp_mss);
		fflush(where);
#ifdef WANT_HISTOGRAM
...
void send_sdp_maerts(char remote_host[])
{
...
%6d\n";
	...
	sdp_maerts_request =
		(struct sdp_maerts_request_struct *)netperf_request.content.
		test_specific_data;
	sdp_maerts_response =
		(struct sdp_maerts_response_struct *)netperf_response.content.
		test_specific_data;
	sdp_maerts_result =
		(struct sdp_maerts_results_struct *)netperf_response.content.
		test_specific_data;
...
void recv_sdp_stream()
{
	...
	if (debug) {
		fprintf(where,
				"recv_sdp_stream: receive alignment and offset set...\n");
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_sdp_stream: got %g bytes\n", bytes_received);
		fprintf(where, "recv_sdp_stream: got %d recvs\n", receive_calls);
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_sdp_stream: test complete, sending results.\n");
		fprintf(where,
...
void recv_sdp_maerts()
{
	...
	if (debug) {
		fprintf(where,
				"recv_sdp_maerts: receive alignment and offset set...\n");
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_sdp_maerts: got %g bytes\n", bytes_sent);
		fprintf(where, "recv_sdp_maerts: got %d sends\n", send_calls);
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_sdp_maerts: test complete, sending results.\n");
		fprintf(where,
...
void send_sdp_rr(char remote_host[])
{
...
#define REQUEST_CWND_INITIAL 2
	...
	sdp_rr_request =
		(struct sdp_rr_request_struct *)netperf_request.content.
		test_specific_data;
	sdp_rr_response =
		(struct sdp_rr_response_struct *)netperf_response.content.
		test_specific_data;
	sdp_rr_result =
		(struct sdp_rr_results_struct *)netperf_response.content.
		test_specific_data;
	...
	if (print_headers) {
		print_top_test_header("SDP REQUEST/RESPONSE TEST", local_res,
							  remote_res);
	}
	...
	while (((confidence < 0) && (confidence_iteration < iteration_max)) ||
		...
		if (!no_control) {
			...
			if (debug > 1) {
				fprintf(where,
						"netperf: send_sdp_rr: requesting SDP rr test\n");
			}
			...
			if (!netperf_response.content.serv_errno) {
				...
				set_port_number(remote_res,
								(short)sdp_rr_response->data_port_number);
			} else {
				Set_errno(netperf_response.content.serv_errno);
				fprintf(where,
						"netperf: remote error %d",
...
void recv_sdp_rr()
{
	...
	if (debug) {
		fprintf(where,
				"recv_sdp_rr: requested recv alignment of %d offset %d\n",
				sdp_rr_request->recv_alignment, sdp_rr_request->recv_offset);
		fprintf(where,
				"recv_sdp_rr: requested send alignment of %d offset %d\n",
				sdp_rr_request->send_alignment, sdp_rr_request->send_offset);
		fflush(where);
	}
	...
	if (sdp_rr_request->measure_cpu) {
		...
		sdp_rr_response->cpu_rate =
			calibrate_local_cpu(sdp_rr_request->cpu_rate);
	}
	...
	if (debug) {
		fprintf(where,
				"recv_sdp_rr: accept completes on the data connection.\n");
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_sdp_rr: got %d transactions\n", trans_received);
		fflush(where);
	}
	...
	if (debug) {
		fprintf(where, "recv_sdp_rr: test complete, sending results.\n");
		fflush(where);
	}

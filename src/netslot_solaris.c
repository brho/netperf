...
static char *set_interface_match(char *interface_name)
{
...
static int parse_binding_name(char *binding_name)
{
...
static int check_node(di_node_t node, void *arg)
{
	...
	while ((minor = di_minor_next(node, minor)) != DI_MINOR_NIL) {
		...
		if (strcmp(minorname, interface_match) == 0) {
			...
			printf("FOUND A MATCH ON %s under node %s with binding name %s\n",
				   interface_match, nodename, bindingname);
#endif
...
void
find_interface_ids(char *interface_name, int *vendor, int *device,
...
int main(int argc, char *argv[])
{
	...
	printf
		("%s in in slot %s: vendor %4x device %4x subvendor %4x subdevice %4x\n",
		 argv[1], slot, vendor, device, subvendor, subdevice);

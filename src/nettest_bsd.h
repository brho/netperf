...
extern int
 ...
 rsr_size_end, rss_size_end, lss_size_req,	/* requested local socket send buffer size */
 ...
 lss_size_end, lsr_size_end, req_size,	/* request size */
...
extern void set_port_number(struct addrinfo *res, unsigned short port);
extern void set_hostname_and_port(char *hostname,
								  char *portstr, int family, int port);
extern void set_sockaddr_family_addr_port(struct sockaddr_storage *sockaddr,
										  int family, void *addr, int port);
